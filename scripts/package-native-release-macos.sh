...
package_scripts_root="$output_root/.macos-scripts"

rm -rf "$stage_root" "$payload_root" "$package_scripts_root"
...
mkdir -p "$payload_root/usr/local/lib/vektor-flow" "$payload_root/usr/local/bin" "$package_scripts_root"
...
printf '%s\n' "Vektor Flow managed installation" > "$payload_root/usr/local/lib/vektor-flow/.vektor-flow-install"
ln -s ../lib/vektor-flow/bin/vkf "$payload_root/usr/local/bin/vkf"
cat > "$package_scripts_root/preinstall" <<'EOF'
#!/bin/sh
set -eu

install_root=/usr/local/lib/vektor-flow
command_path=/usr/local/bin/vkf

if [ -e "$install_root" ] && [ ! -d "$install_root" ]; then
  echo "Vektor Flow: refusing to replace non-directory $install_root" >&2
  exit 2
fi
if [ -d "$install_root" ] && [ -n "$(ls -A "$install_root" 2>/dev/null)" ] &&
   [ ! -f "$install_root/.vektor-flow-install" ] &&
   [ ! -f "$install_root/vektorflow-release.json" ]; then
  echo "Vektor Flow: refusing to replace an installation directory owned by another application" >&2
  exit 2
fi
if [ -e "$command_path" ] || [ -L "$command_path" ]; then
  if [ ! -L "$command_path" ]; then
    echo "Vektor Flow: refusing to replace existing command $command_path" >&2
    exit 2
  fi
  current_target=$(readlink "$command_path")
  case "$current_target" in
    ../lib/vektor-flow/bin/vkf|/usr/local/lib/vektor-flow/bin/vkf) ;;
    *)
      echo "Vektor Flow: refusing to replace a vkf link owned by another installation" >&2
      exit 2
      ;;
  esac
fi
EOF
chmod +x "$package_scripts_root/preinstall"
pkgbuild \
  --root "$payload_root" \
  --scripts "$package_scripts_root" \
  --identifier com.vektorflow.compiler \
  --version "$version" \
  --install-location / \
  "$package_path"
(cd "$output_root" && shasum -a 256 "$(basename "$package_path")" > "$(basename "$package_path").sha256")
rm -rf "$payload_root" "$package_scripts_root"
