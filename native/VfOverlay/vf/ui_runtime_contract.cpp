...
GeomColorPatchPacketPayload parse_geom_color_patch_payload(const JsonValue::Object& payload) {
    require_allowed_keys(payload, {"frame_id", "object_id", "color"}, "geom.color.patch payload");
    GeomColorPatchPacketPayload out;
    out.frame_id = require_object_field(payload, "frame_id").as_string();
    out.object_id = require_uint64(require_object_field(payload, "object_id"), "object_id");
    out.color = require_object_field(payload, "color");
    return out;
}
...
UiRuntimePacketKind ParseUiRuntimePacketKind(std::string_view kind) {
    ...
    if (kind == "geom.color.patch") {
        return UiRuntimePacketKind::GeomColorPatch;
    }
...
JsonValue ToJsonValue(const GeomColorPatchPacketPayload& payload) {
    return JsonValue::Object{
        ...
        {"object_id", JsonValue(static_cast<double>(payload.object_id))},
    };
}
...
const GeomColorPatchPacketPayload* AsGeomColorPatchPacketPayload(const UiRuntimePacket& packet) noexcept {
    return std::get_if<GeomColorPatchPacketPayload>(&packet.payload);
}
...
void ValidateUiRuntimePacket(const UiRuntimePacket& packet) {
    ...
    case UiRuntimePacketKind::GeomColorPatch: {
        const auto* payload = AsGeomColorPatchPacketPayload(packet);
        if (payload == nullptr) {
            throw std::runtime_error("geom.color.patch packet kind/payload mismatch");
        }
        if (payload->frame_id.empty()) {
            throw std::runtime_error("geom.color.patch frame_id must not be empty");
        }
        if (payload->object_id == 0) {
            throw std::runtime_error("geom.color.patch object_id must be greater than zero");
        }
        static_cast<void>(ToJsonValue(*payload));
        break;
    }
...
UiRuntimePacket ParseUiRuntimePacket(const JsonValue& value) {
    ...
    case UiRuntimePacketKind::DisplayReplace:
        ...
        break;
    case UiRuntimePacketKind::GeomColorPatch:
        packet.payload = parse_geom_color_patch_payload(payload);
        break;
    case UiRuntimePacketKind::WidgetAppendText:
