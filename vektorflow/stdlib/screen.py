...
from vektorflow.ui_frame_commands import (
    apply_widget_meta,
    attach_widget_event_consts,
    build_frame_upsert_command,
    merge_widget_state,
...
class _Widget:
    ...
    def label(self, id: str, text: str = "", **kwargs: Any) -> dict[str, Any]:
        ...
        return attach_widget_event_consts(
            apply_widget_meta({"id": str(id), "type": "label", "text": str(text)}, grid=grid)
        )
    ...
    def button(self, id: str, label: str = "", **kwargs: Any) -> dict[str, Any]:
        ...
        return attach_widget_event_consts(
            apply_widget_meta({"id": str(id), "type": "button", "label": str(label)}, grid=grid)
        )
    ...
    def text_area(self, id: str, text: str = "", **kwargs: Any) -> dict[str, Any]:
        ...
        return attach_widget_event_consts(
            apply_widget_meta({"id": str(id), "type": "textarea", "text": str(text)}, grid=grid)
        )
    ...
    def dropdown(
        ...
        return attach_widget_event_consts(
            apply_widget_meta({"id": str(id), "type": "dropdown", "options": opts, "value": int(value)}, grid=grid)
        )
...
class Screen:
    ...
    def frame(
        ...
        **kwargs: Any,
        ...
        return PendingFrame(
            ...
            body_layout=(
                normalize_grid_layout(gridlayout if gridlayout is not None else grid_layout)
    ...
    def add_frame(
        ...
        **Master:** if ``pending.master`` is true, closing that frame in the host UI
        ...
        fid = self._alloc_id()
        plan = build_frame_upsert_command(
            fid,
            title=pending.title,
            ...
            body=body,
            body_layout=pending.body_layout,
            parent_id=parent_id,
        )
        self._commands.append(plan.command)
    ...
    def dumps(self) -> str:
        ...
        return dumps_scene(self._commands)
    ...
    def widget_set(self, frame_id: str, widget_id: str, props: Any) -> None:
        ...
        self._ui_state = merge_widget_state(
            self._ui_state,
...
def _write_vkf_scene_to_vf_ui(commands: list[UiCommand]) -> None:
    """Write scene JSON for ``vkf-scene.html`` (repo copy + overlay ``web/`` next to built exe)."""
    try:
        ...
        out = root / "web" / "vf-ui" / "vkf-scene.json"
        out.parent.mkdir(parents=True, exist_ok=True)
        out.write_text(text, encoding="utf-8")
        _sync_json_to_all_built_webs(root, "vkf-scene.json", text)
...
def _write_vf_ui_state_to_vf_ui(state: dict[str, dict[str, dict[str, Any]]]) -> None:
    ...
    try:
        ...
        out = root / "web" / "vf-ui" / "vf-ui-state.json"
        out.parent.mkdir(parents=True, exist_ok=True)
        out.write_text(text, encoding="utf-8")
        exe = find_vf_overlay_exe(root)
        if exe is not None:
            built = exe.parent / "web" / "vf-ui-state.json"
...
def _coerce_norm_rect(rect: Any) -> NormRect:
    if isinstance(rect, (list, tuple)) and len(rect) == 4:
        ...
        return r
    raise TypeError(
        "rect must be a 4-tuple (or list) of numbers (x, y, w, h) in normalized coords"
    )
