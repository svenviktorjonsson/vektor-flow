...
const frameId = 'dry_sand_outlet_regimes_frame';
const blocked = createDrySandHopperReference({
  seed: 0x3130, grainCount: 128, outletDiameterInGrains: 1.8,
});
const flowing = createDrySandHopperReference({
  seed: 0x3130, grainCount: 128, outletDiameterInGrains: 4.2,
});
stepDrySandHopperReference(blocked, 180);
stepDrySandHopperReference(flowing, 180);
...
for (const [world, x, suffix, objectBase] of [
  [blocked, -0.92, 'blocked', 510], [flowing, 0.92, 'flowing', 520],
]) {
...
const ground = Object.freeze({
  type:'field_mesh',id:'sand:outlet-regimes:ground',object_id:500,mode3d:true,
...
try {
  ...
  const panel = window.VfFrame.mount(document.getElementById('layer'), {
    id:frameId,title:'1.8D persistent arch (left) / 4.2D discharge (right)',
  ...
  window.__drySandOutletEvidence={blocked,flowing,diagnostics:[blocked.flowDiagnostic,flowing.flowDiagnostic]};
