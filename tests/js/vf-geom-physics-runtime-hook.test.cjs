...
assert.match(source, /sleepLinearThreshold/);
assert.match(source, /sleepAngularThreshold/);
assert.match(source, /sleepDelay/);
...
assert.match(polygonShader, /contact_geometry/);
assert.match(polygonShader, /restitution_threshold/);
assert.match(polygonShader, /candidate_l/);
assert.match(polygonShader, /rolling_friction/);
assert.match(polygonShader, /let has_contact = sleep_body\.triangle_range\.w < -0\.5/);
assert.match(polygonShader, /sleep_body\.triangle_range\.z >= params\.padding\.w/);
...
assert.match(nativeStager, /sleep_linear_threshold/);
assert.match(nativeStager, /sleep_angular_threshold/);
assert.match(nativeStager, /sleep_delay/);
...
assert.ok(vkfNumbers("e_n").every((value) => value >= 0.90));
assert.ok(vkfNumbers("e_t").every((value) => value >= 0.80));
assert.ok(vkfNumbers("mu_s").every((value) => value <= 0.20));
assert.ok(vkfNumbers("mu_d").every((value) => value <= 0.10));
assert.ok(vkfNumbers("mu_r").every((value) => value <= 0.01));
assert.ok(vkfNumbers("linear_angular_damping").every((value) => value <= 0.0002));
assert.ok(vkfNumbers("restitution_threshold").every((value) => value >= 0.5));
