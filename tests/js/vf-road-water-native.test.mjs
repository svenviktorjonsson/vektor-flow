...
function compare(wear, sampleBudget, waterIdentity = identity) {
  ...
  const input = [wear.sampleCount, sampleBudget, wear.potentialCellCount, ...stream.key, ...stream.counterPrefix,
    ...
    ...bits(wear.drivers), ...bits(wear.material.albedo), ...bits(wear.material.roughness), ...bits(wear.material.wetness)].join(' ');
  const native = spawnSync(executable, [], { input, encoding: 'utf8', timeout: 30000, maxBuffer: 16 * 1024 * 1024 });
  let result;
  try {
  ...
  const expected = [result.sampleCount, result.vectorBytes, Number(result.truncated),
