...
test("scene gallery captures layered 2D bands", async () => {
  const manifest = JSON.parse(await readFile(path.join(galleryRoot, "manifest.json"), "utf8"));
  const example = manifest.examples.find(({ id }) => id === "14-layered-bands");
  assert.ok(example);
  assert.deepEqual(example.features, ["2d", "plot", "layers", "color"]);
  const source = await readFile(path.join(galleryRoot, example.source), "utf8");
  assert.ok(source.includes('id:"lower_band"'));
  assert.ok(source.includes('id:"middle_band"'));
  assert.ok(source.includes('id:"upper_band"'));
  assert.ok(source.match(/frame\.add\(/gu)?.length >= 3);
  assert.equal(sha256(Buffer.from(source.replaceAll("\r\n", "\n"))), example.sourceSha256);
  const bytes = await readFile(path.join(repositoryRoot, example.media.path));
  assert.ok(bytes.length > 100);
  assert.equal(sha256(bytes), example.media.sha256);
...
test("scene gallery captures a focused spot light", async () => {
  const manifest = JSON.parse(await readFile(path.join(galleryRoot, "manifest.json"), "utf8"));
  const example = manifest.examples.find(({ id }) => id === "15-spot-light");
  assert.ok(example);
  assert.deepEqual(example.features, ["3d", "lighting", "spot-light", "shadows"]);
  const source = await readFile(path.join(galleryRoot, example.source), "utf8");
  for (const field of ['kind:"spot"', "inner_cone_deg:12", "outer_cone_deg:24", "casts_shadow:true"]) {
    assert.ok(source.includes(field), `missing ${field}`);
  }
  assert.equal(sha256(Buffer.from(source.replaceAll("\r\n", "\n"))), example.sourceSha256);
  const bytes = await readFile(path.join(repositoryRoot, example.media.path));
...
test("scene gallery captures a procedural dice texture", async () => {
  const manifest = JSON.parse(await readFile(path.join(galleryRoot, "manifest.json"), "utf8"));
  const example = manifest.examples.find(({ id }) => id === "16-dice-texture");
  assert.ok(example);
  assert.deepEqual(example.features, ["3d", "texture", "procedural", "shadows"]);
  const source = await readFile(path.join(galleryRoot, example.source), "utf8");
