...
const component = "machine_ir.closed_dynamic_arithmetic.typed_module_pipeline";
...
test("Stage 2 compiler CLI preserves unary-minus precedence and repetition", {
  ...
  const work = mkdtempSync(join(rootWork, "i178-stage2-unbounded-unary-"));
  try {
    ...
    const inputSource = join(work, "unbounded-unary-input.vkf");
    const oracleArtifact = join(work, `unbounded-unary-oracle${suffix}`);
    writeFileSync(
      inputSource,
    ...
    assert.equal(oracleRun.stdout, `154${newline}`);

    const stage2Artifact = join(work, `stage2-unbounded-unary${suffix}`);
    const observation = join(work, "stage2-unbounded-unary-observation.txt");
    const provenance = join(work, "stage2-unbounded-unary-provenance.json");
    const cliSource = join(work, "stage2-unbounded-unary-compiler.vkf");
    const cliArtifact = join(work, `stage2-unbounded-unary-compiler${suffix}`);
    writeFileSync(cliSource, [
      ...
      "statement: compiler_stage.compile_tagged_printed_dynamic_grouped_arithmetic(source)",
    ...
    const observationLines = readFileSync(observation, "utf8").trim().split(/\r?\n/);
    assert.equal(observationLines[5], "4", "unary precedence stack maximum changed");
    assert.match(observationLines[7], /\b10\b/, "unary minus did not lower to its private opcode");
    ...
    writeFileSync(
      inputSource,
      "value: 100\n:: other + 10 * (-2 ^ 2) + 3 * ((-2) ^ 2) + --2\n",
      "utf8",
    );
    const rejected = runCli();
    assert.notEqual(rejected.status, 0, "unknown unary binding was accepted");
    assert.deepEqual(readFileSync(stage2Artifact), firstArtifact);
    assert.deepEqual(readFileSync(provenance), firstProvenance);
  } finally {
