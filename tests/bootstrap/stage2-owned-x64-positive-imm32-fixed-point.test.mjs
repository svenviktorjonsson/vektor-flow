...
test("Stage 2 encodes an ASCII-safe positive imm32 at fixed point", {
  ...
  const work = mkdtempSync(join(rootWork, "i196-stage2-x64-positive-imm32-"));
  try {
    ...
    const inputSource = join(work, "valid-positive-imm32.vkf");
    const oracleArtifact = join(work, `oracle${suffix}`);
    writeFileSync(inputSource, "value: 2130772480\n:: value + 1\n", "utf8");
    ...
    assert.equal(oracle.stdout, `2130772481${newline}`);

    const stage2CompilerSource = join(work, "stage2-positive-imm32-compiler.vkf");
    const stage2Compiler = join(work, `stage2-positive-imm32-compiler${suffix}`);
    ...
    const stage2Program = join(work, `stage2-positive-imm32${suffix}`);
    const stage3Compiler = join(work, `stage3-positive-imm32-compiler${suffix}`);
    ...
    const stage3Program = join(work, `stage3-positive-imm32${suffix}`);
    const stage4Compiler = join(work, `stage4-positive-imm32-compiler${suffix}`);
    ...
    assert.deepEqual(
      ...
      Buffer.from([
        0x68, 0x00, 0x02, 0x01, 0x7f,
        0x6a, 0x01,
        0x58, 0x59, 0x48, 0x01, 0xc8, 0x50,
        0x58, 0xf2, 0x48, 0x0f, 0x2a, 0xc0, 0xc3,
      ]),
      "Stage 2 did not encode the positive imm32 little-endian",
    );
  } finally {
