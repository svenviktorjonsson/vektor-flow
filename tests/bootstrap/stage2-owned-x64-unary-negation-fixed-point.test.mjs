...
test("Stage 2 emits unary negation at x64 fixed point", {
  ...
  const work = mkdtempSync(join(rootWork, "i214-"));
  try {
    ...
    assert.equal(oracle.stdout, `-8${newline}`);
    ...
    writeFileSync(stage2CompilerSource, [
      ...
      "artifact: compiler_stage._compile_printed_dynamic_unary_numeric_chain_x64(",
    ...
    assert.deepEqual(
      ...
      generated,
      "unary-negation bytes differ from the selected dynamic tape",
    );
  } finally {
