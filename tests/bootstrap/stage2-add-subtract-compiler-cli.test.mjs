...
const component = "machine_ir.closed_add_subtract.typed_module_pipeline";
...
test("Stage 2 compiler CLI preserves add-subtract order", {
  ...
  const work = mkdtempSync(join(rootWork, "i160-stage2-add-subtract-"));
  try {
    ...
    const inputSource = join(work, "add-subtract-input.vkf");
    const oracleArtifact = join(work, `add-subtract-oracle${suffix}`);
    ...
    const stage2Artifact = join(work, `stage2-add-subtract${suffix}`);
    const observation = join(work, "stage2-add-subtract-observation.txt");
    const provenance = join(work, "stage2-add-subtract-provenance.json");
    const cliSource = join(work, "stage2-add-subtract-compiler.vkf");
    const cliArtifact = join(work, `stage2-add-subtract-compiler${suffix}`);
    ...
    writeFileSync(cliSource, [
      ...
      "module: compiler_stage.compile_tagged_printed_add_subtract(source)",
