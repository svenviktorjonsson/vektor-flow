...
test("Stage 2 compiler CLI preserves power precedence and right associativity", {
  ...
  const work = mkdtempSync(join(rootWork, "i176-stage2-unbounded-power-"));
  try {
    ...
    const inputSource = join(work, "unbounded-power-input.vkf");
    const oracleArtifact = join(work, `unbounded-power-oracle${suffix}`);
    writeFileSync(
      inputSource,
      "value: 10\n:: value + 2 ^ 3 ^ 2 * 2\n",
      "utf8",
    ...
    assert.equal(oracleRun.stdout, `1034${newline}`);

    const stage2Artifact = join(work, `stage2-unbounded-power${suffix}`);
    const observation = join(work, "stage2-unbounded-power-observation.txt");
    const provenance = join(work, "stage2-unbounded-power-provenance.json");
    const cliSource = join(work, "stage2-unbounded-power-compiler.vkf");
    const cliArtifact = join(work, `stage2-unbounded-power-compiler${suffix}`);
