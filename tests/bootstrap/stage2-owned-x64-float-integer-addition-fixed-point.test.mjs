...
test("Stage 2 adds an integer to a division result in its typed numeric stack", {
  ...
  const work = mkdtempSync(join(rootWork, "i201-"));
  try {
    ...
    const floatIntegerAdditionPath = join(work, "float-integer-add.bin");
    ...
    writeFileSync(stage2CompilerSource, [
      ...
      "artifact: compiler_stage._compile_printed_dynamic_typed_numeric_chain_x64(",
    ...
    assert.deepEqual(
      ...
      generated,
      "float-plus-integer bytes differ from the selected typed numeric tape",
    );
  } finally {
