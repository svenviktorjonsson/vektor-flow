...
const component = "machine_ir.numeric_count_to_loop.typed_module_pipeline";
...
test("Stage 2 compiler CLI lowers one count-to loop function and call", {
  ...
  const work = mkdtempSync(join(rootWork, "i153-stage2-loop-"));
  try {
    ...
    const inputSource = join(work, "loop-input.vkf");
    const oracleArtifact = join(work, `loop-oracle${suffix}`);
    copyFileSync(
    ...
    assert.equal(oracleRun.stdout.trim(), "3");

    const machine = JSON.parse(readFileSync(
      join(work, ".vkfbuild", "loop-input", "machine-ir.json"),
      "utf8",
    ));
    ...
    const stage2Artifact = join(work, `stage2-loop${suffix}`);
    const observation = join(work, "stage2-loop-observation.txt");
    const provenance = join(work, "stage2-loop-provenance.json");
    const cliSource = join(work, "stage2-loop-compiler.vkf");
    const cliArtifact = join(work, `stage2-loop-compiler${suffix}`);
    writeFileSync(cliSource, [
      ...
      "module: compiler_stage.compile_tagged_numeric_count_to_loop(source)",
      ...shapeGuards,
    ...
    assert.notEqual(rejected.status, 0, "invalid loop source was accepted");
    assert.deepEqual(readFileSync(stage2Artifact), firstArtifact);
    assert.deepEqual(readFileSync(provenance), firstProvenance);
  } finally {
