...
test("Stage 2 subtracts a unary-negative operand at x64 fixed point", {
  ...
  const work = mkdtempSync(join(rootWork, "i216-"));
  try {
    ...
    assert.equal(oracle.stdout, `11${newline}`);
    ...
    writeFileSync(stage2CompilerSource, [
      ...
      "artifact: compiler_stage._compile_printed_dynamic_signed_subtraction_x64(",
    ...
    assert.deepEqual(
      ...
      generated,
      "signed-subtraction bytes differ from the selected dynamic tape",
    );
  } finally {
