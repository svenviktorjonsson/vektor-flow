...
test("Stage 2 emits a grouped numeric chain", {
  ...
  const work = mkdtempSync(join(rootWork, "i207-"));
  try {
    ...
    const generated = Buffer.concat([
      Buffer.from([0x6a, 0x12, 0x6a, 0x04, 0x6a, 0x02]),
      addition,
      division,
      floatingPrint,
    ]);
    ...
    writeFileSync(inputSource, "value: 18\n:: value / (4 + 2)\n", "utf8");
    ...
    assert.equal(oracle.stdout, `3${newline}`);
    ...
    writeFileSync(stage2CompilerSource, [
      ...
      "artifact: compiler_stage._compile_printed_dynamic_grouped_numeric_chain_x64(",
    ...
    assert.deepEqual(
      ...
      generated,
      "grouped numeric bytes differ from the selected tape",
    );
  } finally {
