...
test("Stage 2 subtracts one grouped floating result from another", {
  ...
  const work = mkdtempSync(join(rootWork, "i211-"));
  try {
    ...
    const floatingSubtractionPath = join(work, "float-subtract.bin");
    ...
    writeFileSync(stage2CompilerSource, [
      ...
      "artifact: compiler_stage._compile_printed_dynamic_grouped_floating_subtraction_numeric_chain_x64(",
    ...
    assert.deepEqual(
      ...
      generated,
      "float-minus-float bytes differ from the selected grouped tape",
    );
  } finally {
