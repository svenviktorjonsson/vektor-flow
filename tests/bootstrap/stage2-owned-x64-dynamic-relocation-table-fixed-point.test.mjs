...
test("Stage 2 applies a dynamic relocation table to three source-owned functions", {
  ...
  const work = mkdtempSync(join(rootWork, "i229-"));
  try {
    ...
    assert.equal(identityHelper.length, 20, "identity helper layout changed");
    const entryOffset = 2 + firstHelper.length + (identityHelper.length * 2);
    const firstCallReturnOffset = entryOffset + 2 + 5;
    const secondCallReturnOffset = firstCallReturnOffset + resultBridge.length + 5;
    const thirdCallReturnOffset = secondCallReturnOffset + resultBridge.length + 5;
    assert.equal(entryOffset, 62, "entry symbol position changed");
    assert.equal(2 - firstCallReturnOffset, -67, "first relocation changed");
    assert.equal(22 - secondCallReturnOffset, -58, "second relocation changed");
    assert.equal(42 - thirdCallReturnOffset, -49, "third relocation changed");
    ...
    writeFileSync(stage2CompilerSource, [
      ...
      "artifact: compiler_stage._compile_tagged_numeric_literal_function_chain_symbols_x64(",
    ...
    assert.deepEqual(
      ...
      generated,
      "dynamic relocation table bytes differ from the selected artifact",
    );
  } finally {
