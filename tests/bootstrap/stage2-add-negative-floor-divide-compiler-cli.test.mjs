...
const component = "machine_ir.closed_add_floor_divide.typed_module_pipeline";
...
test("Stage 2 compiler CLI preserves signed floor-division precedence", {
  ...
  const work = mkdtempSync(join(rootWork, "i165-stage2-negative-floor-divide-"));
  try {
    ...
    const inputSource = join(work, "negative-floor-divide-input.vkf");
    const oracleArtifact = join(work, `negative-floor-divide-oracle${suffix}`);
    ...
    assert.equal(oracleRun.stdout, `27${newline}`);

    const stage2Artifact = join(work, `stage2-negative-floor-divide${suffix}`);
    const observation = join(work, "stage2-negative-floor-divide-observation.txt");
    const provenance = join(work, "stage2-negative-floor-divide-provenance.json");
    const cliSource = join(work, "stage2-negative-floor-divide-compiler.vkf");
    const cliArtifact = join(work, `stage2-negative-floor-divide-compiler${suffix}`);
    const observationExpression = [
      ...
      "module.entry.instructions.2.kind", "module.entry.instructions.2.value",
      "module.entry.instructions.3.kind", "module.entry.instructions.4.kind",
    ...
    writeFileSync(cliSource, [
      ...
      "module: compiler_stage.compile_tagged_printed_add_negative_floor_divide(source)",
    ...
    assert.notEqual(rejected.status, 0, "unknown signed floor-division binding was accepted");
    assert.deepEqual(readFileSync(stage2Artifact), firstArtifact);
    assert.deepEqual(readFileSync(provenance), firstProvenance);
  } finally {
