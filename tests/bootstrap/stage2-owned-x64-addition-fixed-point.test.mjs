...
test("Stage 2 emits runtime x64 addition at bounded fixed point", {
  ...
  const work = mkdtempSync(join(rootWork, "i182-stage2-x64-addition-"));
  try {
    ...
    const tailPath = join(work, "addition-tail.bin");
    ...
    const inputSource = join(work, "valid-addition.vkf");
    ...
    assert.equal(oracle.stdout, `90${newline}`);

    const stage2CompilerSource = join(work, "stage2-addition-compiler.vkf");
    const stage2Compiler = join(work, `stage2-addition-compiler${suffix}`);
    writeFileSync(stage2CompilerSource, [
      ...
      "artifact: compiler_stage._compile_printed_dynamic_addition_expression_x64(",
    ...
    const stage2Program = join(work, `stage2-addition${suffix}`);
    const stage3Compiler = join(work, `stage3-addition-compiler${suffix}`);
    ...
    const stage3Program = join(work, `stage3-addition${suffix}`);
    const stage4Compiler = join(work, `stage4-addition-compiler${suffix}`);
    ...
    assert.deepEqual(
      readFileSync(stage2Program).subarray(markerOffset, markerOffset + 15),
      Buffer.from([
        0x6a, 0x28, 0x6a, 0x32,
        0x58, 0x59, 0x48, 0x01, 0xc8,
        0xf2, 0x48, 0x0f, 0x2a, 0xc0, 0xc3,
      ]),
      "Stage 2 did not emit the runtime addition instruction sequence",
    );
  } finally {
