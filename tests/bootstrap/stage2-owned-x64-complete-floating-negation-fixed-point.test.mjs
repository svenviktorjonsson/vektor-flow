...
test("Stage 2 negates a floating division result through the complete x64 writer", {
  ...
  const work = mkdtempSync(join(rootWork, "i220-"));
  try {
    ...
    const integerNegatePath = join(work, "integer-negate.bin");
    const floatingNegatePath = join(work, "floating-negate.bin");
    ...
    const integerNegate = Buffer.from([0x58, 0x48, 0xf7, 0xd8, 0x50]);
    const floatingNegate = Buffer.from([
      0x58,
    ...
    const generated = Buffer.concat([
      ...
      division,
      floatingNegate, floatingNegate, floatingNegate, floatingNegate, floatingNegate,
      floatingPrint,
    ]);
    ...
    writeFileSync(integerNegatePath, integerNegate);
    writeFileSync(floatingNegatePath, floatingNegate);
    ...
    assert.equal(oracle.stdout, `-4${newline}`);
    ...
    writeFileSync(stage2CompilerSource, [
      ...
      "integer_negate_path: io_stage.read_line()",
      "floating_negate_path: io_stage.read_line()",
      ...
      "artifact: compiler_stage._compile_printed_dynamic_numeric_chain_x64(",
      ...
      arenaReads.join(",\n"),
      "    ],",
      "    io_stage.read_bytes(integer_negate_path),",
      "    io_stage.read_bytes(floating_negate_path)",
      ")",
    ...
    assert.deepEqual(
      ...
      generated,
      "complete-writer floating-negation bytes differ from the selected tape",
    );
  } finally {
