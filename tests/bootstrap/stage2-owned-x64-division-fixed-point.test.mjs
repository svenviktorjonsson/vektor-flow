...
test("Stage 2 emits runtime x64 division at bounded fixed point", {
  ...
  const work = mkdtempSync(join(rootWork, "i185-stage2-x64-division-"));
  try {
    ...
    const tailPath = join(work, "division-tail.bin");
    ...
    const inputSource = join(work, "valid-division.vkf");
    ...
    assert.equal(oracle.stdout, `2.25${newline}`);

    const stage2CompilerSource = join(work, "stage2-division-compiler.vkf");
    const stage2Compiler = join(work, `stage2-division-compiler${suffix}`);
    writeFileSync(stage2CompilerSource, [
      ...
      "artifact: compiler_stage._compile_printed_dynamic_division_expression_x64(",
    ...
    const stage2Program = join(work, `stage2-division${suffix}`);
    const stage3Compiler = join(work, `stage3-division-compiler${suffix}`);
    ...
    const stage3Program = join(work, `stage3-division${suffix}`);
    const stage4Compiler = join(work, `stage4-division-compiler${suffix}`);
    ...
    assert.deepEqual(
      readFileSync(stage2Program).subarray(markerOffset, markerOffset + 21),
      Buffer.from([
        0x6a, 0x5a, 0x6a, 0x28,
        0x58, 0xf2, 0x48, 0x0f, 0x2a, 0xc8,
        0x58, 0xf2, 0x48, 0x0f, 0x2a, 0xc0,
        0xf2, 0x0f, 0x5e, 0xc1, 0xc3,
      ]),
      "Stage 2 did not emit the runtime division instruction sequence",
    );
  } finally {
