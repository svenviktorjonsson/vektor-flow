...
test("Stage 2 compiler CLI crosses the dynamic arithmetic-tape handoff", {
  ...
  const work = mkdtempSync(join(rootWork, "i169-stage2-dynamic-arithmetic-"));
  try {
    ...
    const inputSource = join(work, "dynamic-arithmetic-input.vkf");
    const oracleArtifact = join(work, `dynamic-arithmetic-oracle${suffix}`);
    ...
    assert.equal(oracleRun.stdout, `40${newline}`);

    const stage2Artifact = join(work, `stage2-dynamic-arithmetic${suffix}`);
    const observation = join(work, "stage2-dynamic-arithmetic-observation.txt");
    const provenance = join(work, "stage2-dynamic-arithmetic-provenance.json");
    const cliSource = join(work, "stage2-dynamic-arithmetic-compiler.vkf");
    const cliArtifact = join(work, `stage2-dynamic-arithmetic-compiler${suffix}`);
    writeFileSync(cliSource, [
      ...
      "statement: compiler_stage.compile_tagged_printed_dynamic_arithmetic(source)",
    ...
    assert.notEqual(rejected.status, 0, "unknown dynamic-arithmetic binding was accepted");
    assert.deepEqual(readFileSync(stage2Artifact), firstArtifact);
    assert.deepEqual(readFileSync(provenance), firstProvenance);
  } finally {
