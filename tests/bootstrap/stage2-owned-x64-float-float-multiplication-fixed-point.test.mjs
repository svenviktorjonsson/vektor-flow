...
test("Stage 2 multiplies two grouped floating results", {
  ...
  const work = mkdtempSync(join(rootWork, "i209-"));
  try {
    ...
    const floatingMultiplicationPath = join(work, "float-multiply.bin");
    ...
    const floatingMultiplication = Buffer.from([
    ...
    const generated = Buffer.concat([
      Buffer.from([0x6a, 0x08, 0x6a, 0x04]),
      division,
      Buffer.from([0x6a, 0x06, 0x6a, 0x03]),
      division,
    ...
    writeFileSync(floatingMultiplicationPath, floatingMultiplication);
    ...
    writeFileSync(stage2CompilerSource, [
      ...
      "floating_multiplication_path: io_stage.read_line()",
      ...
      "artifact: compiler_stage._compile_printed_dynamic_grouped_floating_multiplication_numeric_chain_x64(",
      "    io_stage.read_text(source_path),",
      "    io_stage.read_bytes(prefix_path),",
      "    io_stage.read_bytes(unused_path),",
      "    io_stage.read_bytes(unused_path),",
      "    io_stage.read_bytes(floating_multiplication_path),",
      "    io_stage.read_bytes(unused_path),",
    ...
    assert.deepEqual(
      ...
      generated,
      "float-times-float bytes differ from the selected grouped tape",
    );
  } finally {
