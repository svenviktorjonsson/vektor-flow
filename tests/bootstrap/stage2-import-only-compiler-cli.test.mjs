...
const component = "machine_ir.empty.typed_module_pipeline";
...
test("Stage 2 compiler CLI lowers one standard import-only module", {
  ...
  const work = mkdtempSync(join(rootWork, "i155-stage2-import-"));
  try {
    ...
    const inputSource = join(work, "import-input.vkf");
    const oracleArtifact = join(work, `import-oracle${suffix}`);
    writeFileSync(inputSource, "system: .system\n", "utf8");
    ...
    assert.equal(oracleRun.stdout, "");

    const machine = JSON.parse(readFileSync(
      join(work, ".vkfbuild", "import-input", "machine-ir.json"),
      "utf8",
    ));
    ...
    const stage2Artifact = join(work, `stage2-import${suffix}`);
    const observation = join(work, "stage2-import-observation.txt");
    const provenance = join(work, "stage2-import-provenance.json");
    const cliSource = join(work, "stage2-import-compiler.vkf");
    const cliArtifact = join(work, `stage2-import-compiler${suffix}`);
    writeFileSync(cliSource, [
      ...
      "module: compiler_stage.compile_tagged_import_only_module(source)",
      ...shapeGuards,
    ...
    assert.equal(first.stdout, oracleRun.stdout);
    ...
    appendFileSync(inputSource, `:: 1${newline}`, "utf8");
    const rejected = runCli();
    assert.notEqual(rejected.status, 0, "import plus statement was accepted");
    assert.deepEqual(readFileSync(stage2Artifact), firstArtifact);
    assert.deepEqual(readFileSync(provenance), firstProvenance);
  } finally {
