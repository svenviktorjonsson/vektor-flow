...
test("VKF rejects a reusable numeric function without a return terminator", () => {
  const work = makeWork("i89-function-terminator-");
  try {
    copyProbeModules(work);
    const machineIrPath = join(work, "machine_ir.vkf");
    const originalMachineIr = readFileSync(machineIrPath, "utf8");
    ...
    assert.notEqual(
      mutatedMachineIr,
      originalMachineIr,
      "function-terminator mutation did not apply",
    );
    writeFileSync(machineIrPath, mutatedMachineIr, "utf8");

    const source = join(work, "function-terminator.vkf");
    const artifact = join(work, `function-terminator${executableSuffix}`);
    writeFileSync(
      source,
      [
        "validation: .machine_ir_validation",
    ...
    assert.equal(run.error, undefined, `function-terminator probe did not start: ${run.error}`);
    assert.notEqual(run.status, 0, "unterminated numeric function produced output");
    assert.equal(run.stdout, "");
  } finally {
    rmSync(work, { recursive: true, force: true });
  }
