...
test("bounded traversal returns a two-statement parse result", () => {
  const rootWork = join(root, ".work");
  mkdirSync(rootWork, { recursive: true });
  const work = mkdtempSync(join(rootWork, "i113-tagged-two-statement-"));
  try {
    ...
    writeFileSync(harness, [
      ...
      "result: parser.parse_tagged_two_statement_result(",
    ...
    assert.deepEqual(executed.stdout.trim().split(/\r?\n/u), [
      "module", "2", "alpha", "42", "beta", "7", "0", "8", "true",
    ]);
  } finally {
