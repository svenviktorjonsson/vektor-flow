...
test("Stage 2 owns bounded x64 code-section layout", {
  ...
  const work = mkdtempSync(join(rootWork, "i230-"));
  try {
    ...
    const artifactSuffixPath = join(work, "artifact-suffix.bin");
    ...
    writeFileSync(stage2CompilerSource, [
      ...
      "artifact_suffix_path: io_stage.read_line()",
      "byte_arena_root: io_stage.read_line()",
      "artifact_path: io_stage.read_line()",
      "next_compiler_path: io_stage.read_line()",
      "artifact: compiler_stage._compile_tagged_numeric_literal_function_chain_section_x64(",
      ...
      "    io_stage.read_bytes(artifact_suffix_path),",
      "    [",
      arenaReads.join(",\n"),
      "    ]",
      ")",
    ...
    assert.doesNotMatch(
      generatedCompilerSource,
      /artifact_tail|code_capacity|32768/,
      "caller must not supply code-section padding or capacity",
    );
