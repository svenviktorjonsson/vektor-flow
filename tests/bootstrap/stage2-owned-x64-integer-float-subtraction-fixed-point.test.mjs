...
test("Stage 2 subtracts a grouped floating result from an integer", {
  ...
  const work = mkdtempSync(join(rootWork, "i210-"));
  try {
    ...
    const integerFloatingSubtractionPath = join(work, "int-float-subtract.bin");
    ...
    const integerFloatingSubtraction = Buffer.from([
    ...
    const generated = Buffer.concat([
      Buffer.from([0x6a, 0x09, 0x6a, 0x06, 0x6a, 0x03]),
      division,
      integerFloatingSubtraction,
      floatingPrint,
    ]);
    ...
    writeFileSync(integerFloatingSubtractionPath, integerFloatingSubtraction);
    ...
    writeFileSync(stage2CompilerSource, [
      ...
      "integer_floating_subtraction_path: io_stage.read_line()",
      ...
      "artifact: compiler_stage._compile_printed_dynamic_grouped_typed_subtraction_numeric_chain_x64(",
      "    io_stage.read_text(source_path),",
      "    io_stage.read_bytes(prefix_path),",
      "    io_stage.read_bytes(unused_path),",
      "    io_stage.read_bytes(unused_path),",
      "    io_stage.read_bytes(unused_path),",
      "    io_stage.read_bytes(integer_floating_subtraction_path),",
    ...
    const compilerInput = [
      ...
      unusedPath,
      divisionPath,
      integerFloatingSubtractionPath,
      floatingPrintPath,
      byteArenaRoot,
    ];
    ...
    assert.deepEqual(
      ...
      generated,
      "integer-minus-float bytes differ from the selected grouped tape",
    );
  } finally {
