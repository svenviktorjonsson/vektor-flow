...
const component = "machine_ir.scalar_binding.typed_module_pipeline";
...
test("Stage 2 compiler CLI lowers one printed scalar binding", {
  ...
  const work = mkdtempSync(join(rootWork, "i156-stage2-binding-"));
  try {
    ...
    const inputSource = join(work, "binding-input.vkf");
    const oracleArtifact = join(work, `binding-oracle${suffix}`);
    writeFileSync(inputSource, "value: 5\n:: value\n", "utf8");
    ...
    assert.equal(oracleRun.stdout, `5${newline}`);

    const machine = JSON.parse(readFileSync(
      join(work, ".vkfbuild", "binding-input", "machine-ir.json"),
      "utf8",
    ));
    ...
    const stage2Artifact = join(work, `stage2-binding${suffix}`);
    const observation = join(work, "stage2-binding-observation.txt");
    const provenance = join(work, "stage2-binding-provenance.json");
    const cliSource = join(work, "stage2-binding-compiler.vkf");
    const cliArtifact = join(work, `stage2-binding-compiler${suffix}`);
    writeFileSync(cliSource, [
      ...
      "module: compiler_stage.compile_tagged_scalar_binding(source)",
      ...shapeGuards,
    ...
    assert.notEqual(rejected.status, 0, "mismatched scalar binding was accepted");
    assert.deepEqual(readFileSync(stage2Artifact), firstArtifact);
    assert.deepEqual(readFileSync(provenance), firstProvenance);
  } finally {
