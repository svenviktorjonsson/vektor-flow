...
test("Stage 2 emits runtime x64 integer power at bounded fixed point", {
  ...
  const work = mkdtempSync(join(rootWork, "i188-stage2-x64-power-"));
  try {
    ...
    const tailPath = join(work, "power-tail.bin");
    const codeTail = Buffer.from([
      0x59,                         // pop rcx (ASCII exponent)
      0x48, 0x83, 0xe9, 0x30,       // sub rcx, 48
      0x58,                         // pop rax (base)
    ...
    const inputSource = join(work, "valid-power.vkf");
    ...
    assert.equal(oracle.stdout, `1600${newline}`);

    const stage2CompilerSource = join(work, "stage2-power-compiler.vkf");
    const stage2Compiler = join(work, `stage2-power-compiler${suffix}`);
    writeFileSync(stage2CompilerSource, [
      ...
      "artifact: compiler_stage._compile_printed_dynamic_power_expression_x64(",
    ...
    const stage2Program = join(work, `stage2-power${suffix}`);
    const stage3Compiler = join(work, `stage3-power-compiler${suffix}`);
    ...
    const stage3Program = join(work, `stage3-power${suffix}`);
    const stage4Compiler = join(work, `stage4-power-compiler${suffix}`);
    ...
    assert.deepEqual(
      readFileSync(stage2Program).subarray(markerOffset, markerOffset + 38),
      Buffer.from([
        0x6a, 0x28, 0x6a, 0x32,
        0x59, 0x48, 0x83, 0xe9, 0x30, 0x58,
      ...
      "Stage 2 did not emit the runtime integer-power instruction sequence",
    );
  } finally {
