...
test("Stage 2 divides a floating result by an integer", {
  ...
  const work = mkdtempSync(join(rootWork, "i206-"));
  try {
    ...
    const floatingDivisionPath = join(work, "float-int-divide.bin");
    ...
    assert.equal(oracle.stdout, `1.125${newline}`);
    ...
    assert.deepEqual(
      ...
      generated,
      "float-divided-by-integer bytes differ from the selected numeric tape",
    );
  } finally {
