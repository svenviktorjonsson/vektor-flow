...
test("Stage 2 compiler CLI lowers one comment-only empty module", {
  ...
  const work = mkdtempSync(join(rootWork, "i154-stage2-empty-"));
  try {
    ...
    const inputSource = join(work, "empty-input.vkf");
    const oracleArtifact = join(work, `empty-oracle${suffix}`);
    copyFileSync(
    ...
    const machine = JSON.parse(readFileSync(
      join(work, ".vkfbuild", "empty-input", "machine-ir.json"),
      "utf8",
    ));
    ...
    const stage2Artifact = join(work, `stage2-empty${suffix}`);
    const observation = join(work, "stage2-empty-observation.txt");
    const provenance = join(work, "stage2-empty-provenance.json");
    const cliSource = join(work, "stage2-empty-compiler.vkf");
    const cliArtifact = join(work, `stage2-empty-compiler${suffix}`);
    writeFileSync(cliSource, [
      ...
      "module: compiler_stage.compile_tagged_empty_module(source)",
      ...shapeGuards,
    ...
    assert.notEqual(rejected.status, 0, "nonempty module source was accepted");
    assert.deepEqual(readFileSync(stage2Artifact), firstArtifact);
    assert.deepEqual(readFileSync(provenance), firstProvenance);
  } finally {
