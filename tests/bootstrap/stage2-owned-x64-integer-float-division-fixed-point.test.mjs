...
test("Stage 2 divides an integer by a grouped floating result", {
  ...
  const work = mkdtempSync(join(rootWork, "i212-"));
  try {
    ...
    const integerFloatingDivisionPath = join(work, "int-float-divide.bin");
    ...
    const integerFloatingDivision = Buffer.from([
    ...
    const generated = Buffer.concat([
      Buffer.from([0x6a, 0x09, 0x6a, 0x08, 0x6a, 0x04]),
      division,
      integerFloatingDivision,
      floatingPrint,
    ]);
    ...
    writeFileSync(integerFloatingDivisionPath, integerFloatingDivision);
    ...
    assert.equal(oracle.stdout, `4.5${newline}`);
    ...
    writeFileSync(stage2CompilerSource, [
      ...
      "integer_floating_division_path: io_stage.read_line()",
      ...
      "artifact: compiler_stage._compile_printed_dynamic_grouped_typed_division_numeric_chain_x64(",
      ...
      "    io_stage.read_bytes(integer_floating_division_path),",
      "    io_stage.read_bytes(unused_path),",
      "    io_stage.read_bytes(unused_path),",
      "    io_stage.read_bytes(unused_path),",
      "    io_stage.read_bytes(unused_path),",
    ...
    assert.deepEqual(
      ...
      generated,
      "integer-divided-by-float bytes differ from the selected grouped tape",
    );
  } finally {
