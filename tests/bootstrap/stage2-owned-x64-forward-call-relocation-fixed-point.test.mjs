...
test("Stage 2 relocates a forward function call in a self-hosted x64 artifact", {
  ...
  const work = mkdtempSync(join(rootWork, "i225-"));
  try {
    ...
    const gapPath = join(work, "entry-function-gap.bin");
    ...
    const gap = Buffer.alloc(129, 0x90);
    assert.equal(gap.length + 1, 130, "resolved forward displacement changed");
    ...
    const generated = Buffer.concat([
      ...
      Buffer.from([0xe8, 0x82, 0x00, 0x00, 0x00]),
      Buffer.from([0xc3]),
      gap,
      functionPrologue,
    ...
    writeFileSync(stage2CompilerSource, [
      ...
      "artifact: compiler_stage._compile_tagged_numeric_literal_function_x64(",
      "    io_stage.read_text(source_path),",
      "    io_stage.read_bytes(prefix_path),",
      "    io_stage.read_bytes(gap_path),",
      "    130,",
    ...
    assert.deepEqual(
      ...
      generated,
      "forward call relocation bytes differ from the selected artifact",
    );
  } finally {
