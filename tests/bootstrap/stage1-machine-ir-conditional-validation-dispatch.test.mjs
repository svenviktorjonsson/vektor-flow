...
const component = "machine_ir.numeric_positive_conditional.stack_validation";
...
function compileComponent(work) {
  ...
  const source = join(work, "conditional-validator.vkf");
  const artifact = join(work, `conditional-validator${executableSuffix}`);
  writeFileSync(
    source,
    [
...
test("strict Stage dispatch selects the source-owned fixed-conditional stack validator", () => {
  const work = makeWork("i57-conditional-dispatch-");
  try {
...
test("conditional validation dispatch rejects an oracle mismatch without publishing", () => {
  const work = makeWork("i57-conditional-mismatch-");
  try {
    const { artifact, source } = compileComponent(work);
    const oracle = join(work, "oracle.txt");
    const selected = join(work, "selected.txt");
    ...
    assert.equal(rejected.error, undefined, `dispatcher did not start: ${rejected.error}`);
    assert.notEqual(rejected.status, 0, "mismatched Stage 0 oracle was selected");
    assert.match(rejected.stderr, /observation mismatch/);
