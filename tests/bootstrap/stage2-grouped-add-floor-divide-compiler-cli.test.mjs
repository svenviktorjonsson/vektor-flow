...
const component = "machine_ir.closed_grouped_add_floor_divide.typed_module_pipeline";
...
test("Stage 2 compiler CLI lets grouping override floor-division precedence", {
  ...
  const work = mkdtempSync(join(rootWork, "i167-stage2-grouped-add-floor-divide-"));
  try {
    ...
    const inputSource = join(work, "grouped-add-floor-divide-input.vkf");
    const oracleArtifact = join(work, `grouped-add-floor-divide-oracle${suffix}`);
    ...
    assert.equal(oracleRun.stdout, `19${newline}`);

    const stage2Artifact = join(work, `stage2-grouped-add-floor-divide${suffix}`);
    const observation = join(work, "stage2-grouped-add-floor-divide-observation.txt");
    const provenance = join(work, "stage2-grouped-add-floor-divide-provenance.json");
    const cliSource = join(work, "stage2-grouped-add-floor-divide-compiler.vkf");
    const cliArtifact = join(work, `stage2-grouped-add-floor-divide-compiler${suffix}`);
    const observationExpression = [
      ...
      "module.entry.instructions.2.kind", "module.entry.instructions.3.kind",
      "module.entry.instructions.3.value", "module.entry.instructions.4.kind",
    ...
    writeFileSync(cliSource, [
      ...
      "module: compiler_stage.compile_tagged_printed_grouped_add_floor_divide(source)",
    ...
    assert.notEqual(rejected.status, 0, "unknown grouped-add binding was accepted");
    assert.deepEqual(readFileSync(stage2Artifact), firstArtifact);
    assert.deepEqual(readFileSync(provenance), firstProvenance);
  } finally {
