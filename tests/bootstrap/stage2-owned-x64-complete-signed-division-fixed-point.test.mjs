...
test("Stage 2 divides by a unary-negative operand through the complete x64 writer", {
  ...
  const work = mkdtempSync(join(rootWork, "i219-"));
  try {
    ...
    assert.deepEqual(
      ...
      generated,
      "complete-writer signed-division bytes differ from the selected tape",
    );
  } finally {
