...
test("VKF rejects a fixed-conditional condition that leaves values on its false edge", () => {
  const work = makeWork("i74-false-");
  try {
    copyProbeModules(work);
    const machineIrPath = join(work, "machine_ir.vkf");
    const originalMachineIr = readFileSync(machineIrPath, "utf8");
    const mutatedMachineIr = originalMachineIr.replace(
      /            mir_push_f64\(if_statement\.condition\.right\.value\),\r?\n            mir_simple\("ordered_greater_f64"\),\r?\n            mir_branch\("jump_if_false", 1\),\r?\n            mir_push_f64\(then_return\.value\.value\),\r?\n            mir_return_f64\(\)/,
      [
        "            mir_push_f64(if_statement.condition.right.value),",
        '            mir_branch("label", 2),',
        '            mir_branch("jump_if_false", 1),',
    ...
    assert.notEqual(mutatedMachineIr, originalMachineIr, "conditional-false balance mutation did not apply");
    writeFileSync(machineIrPath, mutatedMachineIr, "utf8");

    const source = join(work, "false-balance.vkf");
    const artifact = join(work, `false-balance${executableSuffix}`);
    writeFileSync(
      source,
      [
        "validation: .machine_ir_validation",
    ...
    assert.equal(run.error, undefined, `conditional-false balance probe did not start: ${run.error}`);
    assert.notEqual(run.status, 0, "unbalanced fixed-conditional false edge produced output");
    assert.equal(run.stdout, "");
  } finally {
    rmSync(work, { recursive: true, force: true });
  }
...
test("VKF rejects a fixed conditional whose false target is not a label", () => {
  const work = makeWork("i75-target-");
  try {
    copyProbeModules(work);
    const machineIrPath = join(work, "machine_ir.vkf");
    const originalMachineIr = readFileSync(machineIrPath, "utf8");
    ...
    assert.notEqual(mutatedMachineIr, originalMachineIr, "conditional-target kind mutation did not apply");
    writeFileSync(machineIrPath, mutatedMachineIr, "utf8");

    const source = join(work, "target-kind.vkf");
    const artifact = join(work, `target-kind${executableSuffix}`);
    writeFileSync(
      source,
      [
        "validation: .machine_ir_validation",
    ...
    assert.equal(run.error, undefined, `conditional-target probe did not start: ${run.error}`);
    assert.notEqual(run.status, 0, "non-label fixed-conditional target produced output");
    assert.equal(run.stdout, "");
  } finally {
    rmSync(work, { recursive: true, force: true });
  }
...
test("VKF rejects a fixed conditional whose false edge is not a conditional branch", () => {
  const work = makeWork("i78-source-");
  try {
    copyProbeModules(work);
    const machineIrPath = join(work, "machine_ir.vkf");
    const originalMachineIr = readFileSync(machineIrPath, "utf8");
    ...
    assert.notEqual(mutatedMachineIr, originalMachineIr, "conditional-branch kind mutation did not apply");
    writeFileSync(machineIrPath, mutatedMachineIr, "utf8");

    const source = join(work, "branch-kind.vkf");
    const artifact = join(work, `branch-kind${executableSuffix}`);
    writeFileSync(
      source,
      [
        "validation: .machine_ir_validation",
    ...
    assert.equal(run.error, undefined, `conditional-branch kind probe did not start: ${run.error}`);
    assert.notEqual(run.status, 0, "non-branch fixed-conditional false edge produced output");
    assert.equal(run.stdout, "");
  } finally {
    rmSync(work, { recursive: true, force: true });
  }
...
test("VKF rejects a fixed conditional whose false arm is not terminated by a return", () => {
  const work = makeWork("i82-false-");
  try {
    copyProbeModules(work);
    const machineIrPath = join(work, "machine_ir.vkf");
    const originalMachineIr = readFileSync(machineIrPath, "utf8");
    ...
    assert.notEqual(mutatedMachineIr, originalMachineIr, "false-arm terminator mutation did not apply");
    writeFileSync(machineIrPath, mutatedMachineIr, "utf8");

    const source = join(work, "false-terminator.vkf");
    const artifact = join(work, `false-terminator${executableSuffix}`);
    writeFileSync(
      source,
      [
        "validation: .machine_ir_validation",
    ...
    assert.equal(run.error, undefined, `false-arm terminator probe did not start: ${run.error}`);
    assert.notEqual(run.status, 0, "unterminated fixed-conditional false arm produced output");
    assert.equal(run.stdout, "");
  } finally {
    rmSync(work, { recursive: true, force: true });
  }
...
test("VKF rejects a fixed-conditional entry without a return terminator", () => {
  const work = makeWork("i90-conditional-entry-terminator-");
  try {
    copyProbeModules(work);
    const machineIrPath = join(work, "machine_ir.vkf");
    const originalMachineIr = readFileSync(machineIrPath, "utf8");
    const mutatedMachineIr = originalMachineIr.replace(
      /(_mir_numeric_linked_entry\(function_name:str\):[\s\S]*?_mir_call_no_handler\(function_name, 1, 1, 1\),\r?\n)            mir_return_f64\(\)/,
      '$1            mir_local("store_local", 0)',
    );
    assert.notEqual(
      mutatedMachineIr,
      originalMachineIr,
      "conditional-entry terminator mutation did not apply",
    );
    writeFileSync(machineIrPath, mutatedMachineIr, "utf8");

    const source = join(work, "conditional-entry-terminator.vkf");
    const artifact = join(work, `conditional-entry-terminator${executableSuffix}`);
    writeFileSync(
      source,
      [
        "validation: .machine_ir_validation",
    ...
    assert.equal(run.error, undefined, `conditional-entry terminator probe did not start: ${run.error}`);
    assert.notEqual(run.status, 0, "unterminated fixed-conditional entry produced output");
    assert.equal(run.stdout, "");
  } finally {
    rmSync(work, { recursive: true, force: true });
  }
