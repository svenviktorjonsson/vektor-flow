...
test("Stage 2 emits runtime x64 multiplication at bounded fixed point", {
  ...
  const work = mkdtempSync(join(rootWork, "i184-stage2-x64-multiplication-"));
  try {
    ...
    const tailPath = join(work, "multiplication-tail.bin");
    ...
    const inputSource = join(work, "valid-multiplication.vkf");
    ...
    assert.equal(oracle.stdout, `2000${newline}`);

    const stage2CompilerSource = join(work, "stage2-multiplication-compiler.vkf");
    const stage2Compiler = join(work, `stage2-multiplication-compiler${suffix}`);
    writeFileSync(stage2CompilerSource, [
      ...
      "artifact: compiler_stage._compile_printed_dynamic_multiplication_expression_x64(",
    ...
    const stage2Program = join(work, `stage2-multiplication${suffix}`);
    const stage3Compiler = join(work, `stage3-multiplication-compiler${suffix}`);
    ...
    const stage3Program = join(work, `stage3-multiplication${suffix}`);
    const stage4Compiler = join(work, `stage4-multiplication-compiler${suffix}`);
    ...
    assert.deepEqual(
      readFileSync(stage2Program).subarray(markerOffset, markerOffset + 16),
      Buffer.from([
        0x6a, 0x28, 0x6a, 0x32,
        0x58, 0x59, 0x48, 0x0f, 0xaf, 0xc1,
        0xf2, 0x48, 0x0f, 0x2a, 0xc0, 0xc3,
      ]),
      "Stage 2 did not emit the runtime multiplication instruction sequence",
    );
  } finally {
