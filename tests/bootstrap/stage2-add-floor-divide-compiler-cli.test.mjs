...
test("Stage 2 compiler CLI preserves floor-division precedence", {
  ...
  const work = mkdtempSync(join(rootWork, "i164-stage2-add-floor-divide-"));
  try {
    ...
    const inputSource = join(work, "add-floor-divide-input.vkf");
    const oracleArtifact = join(work, `add-floor-divide-oracle${suffix}`);
    ...
    const stage2Artifact = join(work, `stage2-add-floor-divide${suffix}`);
    const observation = join(work, "stage2-add-floor-divide-observation.txt");
    const provenance = join(work, "stage2-add-floor-divide-provenance.json");
    const cliSource = join(work, "stage2-add-floor-divide-compiler.vkf");
    const cliArtifact = join(work, `stage2-add-floor-divide-compiler${suffix}`);
    ...
    writeFileSync(cliSource, [
      ...
      "module: compiler_stage.compile_tagged_printed_add_floor_divide(source)",
    ...
    assert.notEqual(rejected.status, 0, "unknown floor-division binding was accepted");
    assert.deepEqual(readFileSync(stage2Artifact), firstArtifact);
    assert.deepEqual(readFileSync(provenance), firstProvenance);
  } finally {
