...
test("Stage 2 compiler CLI preserves nested grouping and stack maximum", {
  ...
  const work = mkdtempSync(join(rootWork, "i177-stage2-unbounded-grouped-"));
  try {
    ...
    const inputSource = join(work, "unbounded-grouped-input.vkf");
    const oracleArtifact = join(work, `unbounded-grouped-oracle${suffix}`);
    writeFileSync(
      inputSource,
      "value: 100\n:: (value - (20 + 4) * 2) // (3 + 1)\n",
      "utf8",
    ...
    assert.equal(oracleRun.stdout, `13${newline}`);

    const stage2Artifact = join(work, `stage2-unbounded-grouped${suffix}`);
    const observation = join(work, "stage2-unbounded-grouped-observation.txt");
    const provenance = join(work, "stage2-unbounded-grouped-provenance.json");
    const cliSource = join(work, "stage2-unbounded-grouped-compiler.vkf");
    const cliArtifact = join(work, `stage2-unbounded-grouped-compiler${suffix}`);
    ...
    assert.equal(observationLines[5], "3", "nested grouping stack maximum changed");
    ...
    assert.notEqual(rejected.status, 0, "unknown grouped binding was accepted");
    assert.deepEqual(readFileSync(stage2Artifact), firstArtifact);
    assert.deepEqual(readFileSync(provenance), firstProvenance);
  } finally {
