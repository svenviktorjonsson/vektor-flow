...
test("Stage 2 preserves a zero imm8 byte at fixed point", {
  ...
  const work = mkdtempSync(join(rootWork, "i195-stage2-x64-zero-imm8-"));
  try {
    ...
    const inputSource = join(work, "valid-zero-imm8.vkf");
    const oracleArtifact = join(work, `oracle${suffix}`);
    writeFileSync(inputSource, "value: 65\n:: value * 0 + 40\n", "utf8");
    ...
    assert.equal(oracle.stdout, `40${newline}`);

    const stage2CompilerSource = join(work, "stage2-zero-imm8-compiler.vkf");
    const stage2Compiler = join(work, `stage2-zero-imm8-compiler${suffix}`);
    ...
    const stage2Program = join(work, `stage2-zero-imm8${suffix}`);
    const stage3Compiler = join(work, `stage3-zero-imm8-compiler${suffix}`);
    ...
    const stage3Program = join(work, `stage3-zero-imm8${suffix}`);
    const stage4Compiler = join(work, `stage4-zero-imm8-compiler${suffix}`);
    ...
    assert.deepEqual(
      ...
      "Stage 2 did not preserve the zero immediate byte",
    );
  } finally {
