...
const component = "machine_ir.closed_add_remainder.typed_module_pipeline";
...
test("Stage 2 compiler CLI preserves remainder precedence", {
  ...
  const work = mkdtempSync(join(rootWork, "i162-stage2-add-remainder-"));
  try {
    ...
    const inputSource = join(work, "add-remainder-input.vkf");
    const oracleArtifact = join(work, `add-remainder-oracle${suffix}`);
    ...
    const stage2Artifact = join(work, `stage2-add-remainder${suffix}`);
    const observation = join(work, "stage2-add-remainder-observation.txt");
    const provenance = join(work, "stage2-add-remainder-provenance.json");
    const cliSource = join(work, "stage2-add-remainder-compiler.vkf");
    const cliArtifact = join(work, `stage2-add-remainder-compiler${suffix}`);
    ...
    writeFileSync(cliSource, [
      ...
      "module: compiler_stage.compile_tagged_printed_add_remainder(source)",
    ...
    assert.notEqual(rejected.status, 0, "unknown remainder binding was accepted");
    assert.deepEqual(readFileSync(stage2Artifact), firstArtifact);
    assert.deepEqual(readFileSync(provenance), firstProvenance);
  } finally {
