...
test("Stage 2 multiplies a floating result by an integer", {
  ...
  const work = mkdtempSync(join(rootWork, "i204-"));
  try {
    ...
    const floatingMultiplicationPath = join(work, "float-int-multiply.bin");
    ...
    const generated = Buffer.concat([
      Buffer.from([0x6a, 0x03, 0x6a, 0x04]),
      division,
    ...
    assert.equal(oracle.stdout, `1.5${newline}`);
    ...
    assert.deepEqual(
      ...
      generated,
      "float-times-integer bytes differ from the selected numeric tape",
    );
  } finally {
