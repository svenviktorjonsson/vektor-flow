...
test("Stage 2 subtracts an integer from a floating result", {
  ...
  const work = mkdtempSync(join(rootWork, "i205-"));
  try {
    ...
    const floatingSubtractionPath = join(work, "float-int-subtract.bin");
    ...
    const generated = Buffer.concat([
      Buffer.from([0x6a, 0x09, 0x6a, 0x04]),
      division,
    ...
    writeFileSync(inputSource, "value: 9\n:: value / 4 - 1\n", "utf8");
    ...
    assert.equal(oracle.stdout, `1.25${newline}`);
    ...
    assert.deepEqual(
      ...
      generated,
      "float-minus-integer bytes differ from the selected numeric tape",
    );
  } finally {
