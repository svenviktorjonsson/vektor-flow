...
test("Stage 2 subtracts a negative integer from a floating result through the complete x64 writer", {
  ...
  const work = mkdtempSync(join(rootWork, "i223-"));
  try {
    ...
    const floatingSubtractionPath = join(work, "float-integer-subtraction.bin");
    ...
    const floatingSubtraction = Buffer.from([
    ...
    writeFileSync(floatingSubtractionPath, floatingSubtraction);
    ...
    assert.equal(oracle.stdout, `7${newline}`);
    ...
    writeFileSync(stage2CompilerSource, [
      ...
      "floating_subtraction_path: io_stage.read_line()",
      ...
      "artifact: compiler_stage._compile_printed_dynamic_typed_subtraction_numeric_chain_x64(",
      "    io_stage.read_text(source_path),",
      "    io_stage.read_bytes(prefix_path),",
      "    io_stage.read_bytes(unused_path),",
      "    io_stage.read_bytes(unused_path),",
      "    io_stage.read_bytes(unused_path),",
      "    io_stage.read_bytes(floating_subtraction_path),",
    ...
    assert.deepEqual(
      ...
      generated,
      "complete-writer signed mixed-subtraction bytes differ from the selected tape",
    );
  } finally {
