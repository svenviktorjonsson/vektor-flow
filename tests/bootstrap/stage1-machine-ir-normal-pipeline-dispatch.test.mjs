...
const componentName = "machine_ir.numeric_parameter_multiply.typed_module_pipeline";
...
function prepareTracer(work) {
  ...
  const source = join(work, "typed-module-lowering.vkf");
  const artifact = join(work, `typed-module-lowering${executableSuffix}`);
  writeFileSync(
    source,
    [
      "typed: .typed_ir",
...
test("normal pipeline dispatch consumes the exact VKF typed-module lowering", () => {
  const work = makeWork("i36-ok-");
  try {
    ...
    assert.ok(sourceGraphMarker, "typed-module producer has no source-graph marker");
...
test("normal pipeline rejects mismatch and stale producers without mutation", () => {
  const work = makeWork("i36-reject-");
  try {
    const tracer = prepareTracer(work);
    ...
    writeFileSync(mismatchOracle, tracer.expected.replace(`${outputNewline}2${outputNewline}`, `${outputNewline}3${outputNewline}`), "utf8");
    ...
    const atomic = dispatch({
      ...tracer,
      output: atomicOutput,
      provenance: join(blockedParent, "provenance.json"),
    });
    assert.notEqual(atomic.status, 0, "invalid provenance parent was accepted");
    assert.match(atomic.stderr, /stage component output parent is not a directory/);
    ...
    assert.notEqual(stale.status, 0, "stale producer was consumed");
...
test("normal pipeline rejects malformed matched observations before output", () => {
  const work = makeWork("i36-malformed-");
  try {
    const tracer = prepareTracer(work);
    const malformedSource = join(work, "malformed.vkf");
    const malformedArtifact = join(work, `malformed${executableSuffix}`);
    const malformed = tracer.expected.replace(
      `${outputNewline}multiply_f64${outputNewline}`,
      `${outputNewline}divide_f64${outputNewline}`,
    );
    const validSource = readFileSync(tracer.source, "utf8");
    const malformedSourceText = validSource.replace(
      ":: module.functions.1.instructions.2.kind",
      ':: "divide_f64"',
    );
    assert.notEqual(malformedSourceText, validSource, "opcode mutation did not apply");
    writeFileSync(malformedSource, malformedSourceText, "utf8");
    compile(malformedSource, malformedArtifact);
    const oracle = join(work, "malformed-oracle.txt");
    writeFileSync(oracle, malformed, "utf8");
    const output = join(work, `should-not-exist${executableSuffix}`);
    const provenance = join(work, "should-not-exist.json");
    ...
    assert.notEqual(rejected.status, 0, "malformed matched observation was consumed");
    assert.match(rejected.stderr, /unsupported numeric tracer opcode divide_f64/);
