...
test("Stage 2 divides a floating result by a negative integer through the complete x64 writer", {
  ...
  const work = mkdtempSync(join(rootWork, "i224-"));
  try {
    ...
    const floatingDivisionPath = join(work, "float-integer-division.bin");
    ...
    const floatingDivision = Buffer.from([
    ...
    writeFileSync(floatingDivisionPath, floatingDivision);
    ...
    assert.equal(oracle.stdout, `-2${newline}`);
    ...
    writeFileSync(stage2CompilerSource, [
      ...
      "floating_division_path: io_stage.read_line()",
      ...
      "artifact: compiler_stage._compile_printed_dynamic_typed_division_numeric_chain_x64(",
      "    io_stage.read_text(source_path),",
      "    io_stage.read_bytes(prefix_path),",
      "    io_stage.read_bytes(unused_path),",
      "    io_stage.read_bytes(unused_path),",
      "    io_stage.read_bytes(unused_path),",
      "    io_stage.read_bytes(floating_division_path),",
    ...
    assert.deepEqual(
      ...
      generated,
      "complete-writer signed mixed-division bytes differ from the selected tape",
    );
  } finally {
