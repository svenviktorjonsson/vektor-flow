...
test("Stage 2 selects x64 for a mixed Machine-IR tape at fixed point", {
  ...
  const work = mkdtempSync(join(rootWork, "i189-stage2-x64-mixed-"));
  try {
    ...
    const generatedCodeBytes = 6 + multiplication.length + addition.length + print.length;
    ...
    const inputSource = join(work, "valid-mixed-expression.vkf");
    ...
    assert.equal(oracle.stdout, `3040${newline}`);

    const stage2CompilerSource = join(work, "stage2-mixed-compiler.vkf");
    const stage2Compiler = join(work, `stage2-mixed-compiler${suffix}`);
    writeFileSync(stage2CompilerSource, [
      ...
      "artifact: compiler_stage._compile_printed_dynamic_mixed_expression_x64(",
    ...
    const stage2Program = join(work, `stage2-mixed${suffix}`);
    const stage3Compiler = join(work, `stage3-mixed-compiler${suffix}`);
    ...
    const stage3Program = join(work, `stage3-mixed${suffix}`);
    const stage4Compiler = join(work, `stage4-mixed-compiler${suffix}`);
    ...
    assert.deepEqual(
      readFileSync(stage2Program).subarray(markerOffset, markerOffset + 26),
      Buffer.from([
        0x6a, 0x28, 0x6a, 0x32, 0x6a, 0x3c,
        0x58, 0x59, 0x48, 0x0f, 0xaf, 0xc1, 0x50,
        0x58, 0x59, 0x48, 0x01, 0xc8, 0x50,
        0x58, 0xf2, 0x48, 0x0f, 0x2a, 0xc0, 0xc3,
      ]),
      "Stage 2 did not select the mixed Machine-IR instruction stream",
    );
  } finally {
