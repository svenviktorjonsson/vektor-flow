...
test("Stage 2 compiler CLI parses an unbounded mixed-precedence chain", {
  ...
  const work = mkdtempSync(join(rootWork, "i171-stage2-unbounded-mixed-"));
  try {
    ...
    const inputSource = join(work, "unbounded-mixed-input.vkf");
    const oracleArtifact = join(work, `unbounded-mixed-oracle${suffix}`);
    writeFileSync(
      inputSource,
      "value: 127\n:: value + 31 // 4 // 2 + 9 // 2 + 6\n",
      "utf8",
    ...
    assert.equal(oracleRun.stdout, `140${newline}`);

    const stage2Artifact = join(work, `stage2-unbounded-mixed${suffix}`);
    const observation = join(work, "stage2-unbounded-mixed-observation.txt");
    const provenance = join(work, "stage2-unbounded-mixed-provenance.json");
    const cliSource = join(work, "stage2-unbounded-mixed-compiler.vkf");
    const cliArtifact = join(work, `stage2-unbounded-mixed-compiler${suffix}`);
    ...
    assert.notEqual(rejected.status, 0, "unknown mixed-arithmetic binding was accepted");
    assert.deepEqual(readFileSync(stage2Artifact), firstArtifact);
    assert.deepEqual(readFileSync(provenance), firstProvenance);
  } finally {
