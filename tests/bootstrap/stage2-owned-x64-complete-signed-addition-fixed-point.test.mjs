...
test("Stage 2 adds a unary-negative operand through the complete x64 writer", {
  ...
  const work = mkdtempSync(join(rootWork, "i218-"));
  try {
    ...
    assert.deepEqual(
      ...
      generated,
      "complete-writer signed-addition bytes differ from the selected tape",
    );
  } finally {
