...
test("Stage 2 multiplies by a unary-negative operand at x64 fixed point", {
  ...
  const work = mkdtempSync(join(rootWork, "i217-"));
  try {
    ...
    const printPath = join(work, "print.bin");
    ...
    const generated = Buffer.concat([
      Buffer.from([0x6a, 0x08, 0x6a, 0x03]),
    ...
    writeFileSync(printPath, Buffer.concat([
      print,
    ...
    assert.equal(oracle.stdout, `-24${newline}`);
    ...
    writeFileSync(stage2CompilerSource, [
      ...
      "print_path: io_stage.read_line()",
      "byte_arena_root: io_stage.read_line()",
      "artifact_path: io_stage.read_line()",
      "next_compiler_path: io_stage.read_line()",
      "artifact: compiler_stage._compile_printed_dynamic_signed_multiplication_x64(",
      ...
      "    io_stage.read_bytes(print_path),",
      "    [",
      arenaReads.join(",\n"),
      "    ]",
      ")",
    ...
    assert.deepEqual(
      ...
      generated,
      "signed-multiplication bytes differ from the selected dynamic tape",
    );
  } finally {
