...
test("Stage 2 compiler CLI closes one prior binding", {
  ...
  const work = mkdtempSync(join(rootWork, "i157-stage2-closed-binding-"));
  try {
    ...
    const inputSource = join(work, "closed-binding-input.vkf");
    const oracleArtifact = join(work, `closed-binding-oracle${suffix}`);
    ...
    assert.equal(oracleRun.stdout, `32${newline}`);

    const stage2Artifact = join(work, `stage2-closed-binding${suffix}`);
    const observation = join(work, "stage2-closed-binding-observation.txt");
    const provenance = join(work, "stage2-closed-binding-provenance.json");
    const cliSource = join(work, "stage2-closed-binding-compiler.vkf");
    const cliArtifact = join(work, `stage2-closed-binding-compiler${suffix}`);
    ...
    writeFileSync(cliSource, [
      ...
      "module: compiler_stage.compile_tagged_closed_binding(source)",
    ...
    assert.notEqual(rejected.status, 0, "unknown prior binding was accepted");
    assert.deepEqual(readFileSync(stage2Artifact), firstArtifact);
    assert.deepEqual(readFileSync(provenance), firstProvenance);
  } finally {
