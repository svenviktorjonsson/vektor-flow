...
test("Stage 2 compiler CLI preserves subtraction precedence and associativity", {
  ...
  const work = mkdtempSync(join(rootWork, "i173-stage2-unbounded-subtract-"));
  try {
    ...
    const inputSource = join(work, "unbounded-subtract-input.vkf");
    const oracleArtifact = join(work, `unbounded-subtract-oracle${suffix}`);
    writeFileSync(
      inputSource,
    ...
    assert.equal(oracleRun.stdout, `188${newline}`);

    const stage2Artifact = join(work, `stage2-unbounded-subtract${suffix}`);
    const observation = join(work, "stage2-unbounded-subtract-observation.txt");
    const provenance = join(work, "stage2-unbounded-subtract-provenance.json");
    const cliSource = join(work, "stage2-unbounded-subtract-compiler.vkf");
    const cliArtifact = join(work, `stage2-unbounded-subtract-compiler${suffix}`);
    ...
    writeFileSync(
      inputSource,
      "value: 200\n:: other - 12 * 3 // 2 - 40 // 5 + 7 * 2\n",
      "utf8",
    );
    const rejected = runCli();
    assert.notEqual(rejected.status, 0, "unknown subtraction binding was accepted");
    assert.deepEqual(readFileSync(stage2Artifact), firstArtifact);
    assert.deepEqual(readFileSync(provenance), firstProvenance);
  } finally {
