...
test("tagged cursor assembles an identifier-plus-number expression", () => {
  const rootWork = join(root, ".work");
  mkdirSync(rootWork, { recursive: true });
  const work = mkdtempSync(join(rootWork, "i111-tagged-binary-expression-"));
  try {
    ...
    writeFileSync(harness, [
      ...
      "expression: parser.parse_tagged_binary_expression(left, operator, right)",
      ":: expression.kind",
      ":: expression.op",
    ...
    assert.deepEqual(executed.stdout.trim().split(/\r?\n/u), [
      "binary_op", "+", "identifier", "alpha", "number_literal", "42",
    ]);
  } finally {
