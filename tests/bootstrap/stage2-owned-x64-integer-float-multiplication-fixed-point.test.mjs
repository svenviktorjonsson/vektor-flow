...
test("Stage 2 multiplies an integer by a grouped floating result", {
  ...
  const work = mkdtempSync(join(rootWork, "i208-"));
  try {
    ...
    const integerFloatingMultiplicationPath = join(work, "int-float-multiply.bin");
    ...
    const integerFloatingMultiplication = Buffer.from([
      0x58, 0x66, 0x48, 0x0f, 0x6e, 0xc8,
    ...
    const generated = Buffer.concat([
      Buffer.from([0x6a, 0x03, 0x6a, 0x08, 0x6a, 0x04]),
      division,
      integerFloatingMultiplication,
      floatingPrint,
    ]);
    ...
    writeFileSync(integerFloatingMultiplicationPath, integerFloatingMultiplication);
    ...
    assert.equal(oracle.stdout, `6${newline}`);
    ...
    writeFileSync(stage2CompilerSource, [
      ...
      "integer_floating_multiplication_path: io_stage.read_line()",
      ...
      "artifact: compiler_stage._compile_printed_dynamic_grouped_typed_multiplication_numeric_chain_x64(",
      "    io_stage.read_text(source_path),",
      "    io_stage.read_bytes(prefix_path),",
      "    io_stage.read_bytes(unused_path),",
      "    io_stage.read_bytes(unused_path),",
      "    io_stage.read_bytes(integer_floating_multiplication_path),",
      "    io_stage.read_bytes(unused_path),",
    ...
    const compilerInput = [
      ...
      unusedPath,
      divisionPath,
      integerFloatingMultiplicationPath,
      floatingPrintPath,
      byteArenaRoot,
    ];
    ...
    assert.deepEqual(
      ...
      generated,
      "integer-times-float bytes differ from the selected grouped tape",
    );
  } finally {
