...
test("Stage 2 encodes a valid x64 artifact at bounded fixed point", {
  ...
  const work = mkdtempSync(join(rootWork, "i181-stage2-x64-artifact-"));
  try {
    ...
    const tailPath = join(work, "runner-tail.bin");
    const codeTail = Buffer.from([
      0x58,                   // pop rax
      0xf2, 0x48, 0x0f, 0x2a, 0xc0, // cvtsi2sd xmm0, rax
      0xc3,                   // ret
    ]);
    const generatedCodeBytes = 2 + codeTail.length;
    ...
    const stage2CompilerSource = join(work, "stage2-x64-compiler.vkf");
    const stage2Compiler = join(work, `stage2-x64-compiler${suffix}`);
    writeFileSync(stage2CompilerSource, [
      ...
      "artifact: compiler_stage._compile_printed_dynamic_addition_x64(",
    ...
    const stage2Program = join(work, `stage2-program${suffix}`);
    const stage3Compiler = join(work, `stage3-x64-compiler${suffix}`);
    ...
    const stage3Program = join(work, `stage3-program${suffix}`);
    const stage4Compiler = join(work, `stage4-x64-compiler${suffix}`);
    ...
    assert.deepEqual(
      readFileSync(stage2Program).subarray(markerOffset, markerOffset + 2),
      Buffer.from([0x6a, 0x32]),
      "Stage 2 did not encode push-immediate 50 from the parsed value",
    );
  } finally {
