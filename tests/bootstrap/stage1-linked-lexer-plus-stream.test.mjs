...
test("linked self-hosted lexer emits an identifier-plus stream", () => {
  const rootWork = join(root, ".work");
  mkdirSync(rootWork, { recursive: true });
  const work = mkdtempSync(join(rootWork, "i104-linked-plus-stream-"));
  try {
    ...
    writeFileSync(harness, [
      "lexer: .lexer",
      `observed: lexer.identifier_plus_tokens(\"${fixture}\")`,
      ...["first", "operator", "third"].flatMap((name) => [
        `:: observed.${name}.kind`,
        `:: observed.${name}.value`,
        `:: observed.${name}.line`,
    ...
    const canonical = spawnSync(oracle, ["--file", source, "<i104>"], {
      cwd: work, encoding: "utf8", timeout: 3_000, windowsHide: true,
    });
    assert.equal(canonical.status, 0, canonical.stderr);
    const expected = JSON.parse(canonical.stdout).tokens.slice(0, 3);
