...
test("Stage 2 adds a negative integer to a floating result through the complete x64 writer", {
  ...
  const work = mkdtempSync(join(rootWork, "i221-"));
  try {
    ...
    const floatIntegerAdditionPath = join(work, "float-integer-addition.bin");
    ...
    const floatIntegerAddition = Buffer.from([
      0x58,
      0xf2, 0x48, 0x0f, 0x2a, 0xc8,
      0x58,
    ...
    writeFileSync(floatIntegerAdditionPath, floatIntegerAddition);
    ...
    assert.equal(oracle.stdout, `1${newline}`);
    ...
    writeFileSync(stage2CompilerSource, [
      ...
      "float_integer_addition_path: io_stage.read_line()",
      ...
      "artifact: compiler_stage._compile_printed_dynamic_complete_addition_numeric_chain_x64(",
      "    io_stage.read_text(source_path),",
      "    io_stage.read_bytes(prefix_path),",
      "    io_stage.read_bytes(unused_path),",
      "    io_stage.read_bytes(float_integer_addition_path),",
      "    io_stage.read_bytes(unused_path),",
      "    io_stage.read_bytes(unused_path),",
      "    io_stage.read_bytes(unused_path),",
      "    io_stage.read_bytes(unused_path),",
    ...
    assert.deepEqual(
      ...
      generated,
      "complete-writer signed mixed-addition bytes differ from the selected tape",
    );
  } finally {
