...
test("compiler source assembles every non-entry statement as a module function", () => {
  const rootWork = join(root, ".work");
  mkdirSync(rootWork, { recursive: true });
  const work = mkdtempSync(join(rootWork, "i138-full-module-"));
  try {
    for (const name of [
    ...
    writeFileSync(source, [
      ...
      ":: module.entry.name", ":: module.functions.length()", "",
    ].join("\n"), "utf8");
