...
test("Stage 2 emits runtime x64 subtraction at bounded fixed point", {
  ...
  const work = mkdtempSync(join(rootWork, "i183-stage2-x64-subtraction-"));
  try {
    ...
    const tailPath = join(work, "subtraction-tail.bin");
    ...
    const inputSource = join(work, "valid-subtraction.vkf");
    ...
    assert.equal(oracle.stdout, `50${newline}`);

    const stage2CompilerSource = join(work, "stage2-subtraction-compiler.vkf");
    const stage2Compiler = join(work, `stage2-subtraction-compiler${suffix}`);
    writeFileSync(stage2CompilerSource, [
      ...
      "artifact: compiler_stage._compile_printed_dynamic_subtraction_expression_x64(",
    ...
    const stage2Program = join(work, `stage2-subtraction${suffix}`);
    const stage3Compiler = join(work, `stage3-subtraction-compiler${suffix}`);
    ...
    const stage3Program = join(work, `stage3-subtraction${suffix}`);
    const stage4Compiler = join(work, `stage4-subtraction-compiler${suffix}`);
    ...
    assert.deepEqual(
      readFileSync(stage2Program).subarray(markerOffset, markerOffset + 18),
      Buffer.from([
        0x6a, 0x5a, 0x6a, 0x28,
        0x58, 0x59, 0x48, 0x29, 0xc1,
        0x48, 0x89, 0xc8,
        0xf2, 0x48, 0x0f, 0x2a, 0xc0, 0xc3,
      ]),
      "Stage 2 did not emit the runtime subtraction instruction sequence",
    );
  } finally {
