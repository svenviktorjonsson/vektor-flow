...
test("VKF lowers an exact typed positive conditional application to its whole MachineModule", () => {
  const work = makeWork("i37-conditional-ok-");
  try {
    ...
    const typedFunction = typed.body.find(
      (item) => item.kind === "function" && item.name === "positive",
    );
    ...
    writeFileSync(
      ...
      [
        ...
        "matches: mir.mir_numeric_positive_conditional_typed_module_matches(application)",
        'matches?! "typed positive conditional application does not match MachineModule lowering"',
        ":: matches",
...
test("structurally short typed conditional applications reject at the private layout seam", () => {
  const work = makeWork("i37-conditional-short-");
  try {
    copyProducerModules(work);
    ...
    writeFileSync(
      ...
      [
        ...
        "module: mir.mir_lower_numeric_positive_conditional_typed_module(short)",
        ":: module.schema",
        "",
      ].join("\n"),
      "utf8",
...
test("malformed typed conditional control flow rejects before MachineModule output", () => {
  const work = makeWork("i37-conditional-control-flow-");
  try {
    copyProducerModules(work);
    ...
    assert.notEqual(malformedTyped, originalTyped, "control-flow mutation did not apply");
    writeFileSync(typedPath, malformedTyped, "utf8");

    const source = join(work, "malformed-control-flow.vkf");
    ...
    assert.notEqual(run.status, 0, "malformed control flow unexpectedly produced output");
    assert.equal(run.stdout, "");
    assert.match(
      readFileSync(artifact).toString("latin1"),
      /typed positive conditional application does not match MachineModule lowering/,
    );
  } finally {
    rmSync(work, { recursive: true, force: true });
  }
