...
test("Stage 2 composes terminal true division in its numeric writer", {
  ...
  const work = mkdtempSync(join(rootWork, "i200-"));
  try {
    ...
    const integerPrintPath = join(work, "integer-print.bin");
    ...
    const generated = Buffer.concat([
      Buffer.from([0x6a, 0x5a, 0x6a, 0x02]),
      multiplication,
      Buffer.from([0x6a, 0x28]),
      division,
      floatingPrint,
    ]);
    ...
    writeFileSync(integerPrintPath, Buffer.alloc(0));
    ...
    writeFileSync(stage2CompilerSource, [
      ...
      "integer_print_path: io_stage.read_line()",
      ...
      "    io_stage.read_bytes(unused_path),",
      "    io_stage.read_bytes(unused_path),",
      "    io_stage.read_bytes(integer_print_path),",
    ...
    assert.deepEqual(
      ...
      generated,
      "terminal division bytes differ from the selected Machine-IR tape",
    );
  } finally {
