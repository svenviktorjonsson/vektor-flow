...
const component = "machine_ir.closed_add_power.typed_module_pipeline";
...
test("Stage 2 compiler CLI preserves power precedence", {
  ...
  const work = mkdtempSync(join(rootWork, "i163-stage2-add-power-"));
  try {
    ...
    const inputSource = join(work, "add-power-input.vkf");
    const oracleArtifact = join(work, `add-power-oracle${suffix}`);
    ...
    const stage2Artifact = join(work, `stage2-add-power${suffix}`);
    const observation = join(work, "stage2-add-power-observation.txt");
    const provenance = join(work, "stage2-add-power-provenance.json");
    const cliSource = join(work, "stage2-add-power-compiler.vkf");
    const cliArtifact = join(work, `stage2-add-power-compiler${suffix}`);
    ...
    writeFileSync(cliSource, [
      ...
      "module: compiler_stage.compile_tagged_printed_add_power(source)",
    ...
    assert.notEqual(rejected.status, 0, "unknown power binding was accepted");
    assert.deepEqual(readFileSync(stage2Artifact), firstArtifact);
    assert.deepEqual(readFileSync(provenance), firstProvenance);
  } finally {
