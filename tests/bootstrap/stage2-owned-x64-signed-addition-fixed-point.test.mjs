...
test("Stage 2 adds a unary-negative operand at x64 fixed point", {
  ...
  const work = mkdtempSync(join(rootWork, "i215-"));
  try {
    ...
    assert.equal(oracle.stdout, `5${newline}`);
    ...
    writeFileSync(stage2CompilerSource, [
      ...
      "artifact: compiler_stage._compile_printed_dynamic_signed_addition_x64(",
    ...
    assert.deepEqual(
      ...
      generated,
      "signed-addition bytes differ from the selected dynamic tape",
    );
  } finally {
