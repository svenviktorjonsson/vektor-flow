...
const component = "machine_ir.closed_extended_arithmetic.typed_module_pipeline";
...
test("Stage 2 compiler CLI preserves an extended arithmetic chain", {
  ...
  const work = mkdtempSync(join(rootWork, "i168-stage2-extended-arithmetic-"));
  try {
    ...
    const inputSource = join(work, "extended-arithmetic-input.vkf");
    const oracleArtifact = join(work, `extended-arithmetic-oracle${suffix}`);
    ...
    assert.equal(oracleRun.stdout, `39${newline}`);

    const stage2Artifact = join(work, `stage2-extended-arithmetic${suffix}`);
    const observation = join(work, "stage2-extended-arithmetic-observation.txt");
    const provenance = join(work, "stage2-extended-arithmetic-provenance.json");
    const cliSource = join(work, "stage2-extended-arithmetic-compiler.vkf");
    const cliArtifact = join(work, `stage2-extended-arithmetic-compiler${suffix}`);
    ...
    writeFileSync(cliSource, [
      ...
      "module: compiler_stage.compile_tagged_printed_extended_arithmetic(source)",
    ...
    assert.notEqual(rejected.status, 0, "unknown extended-arithmetic binding was accepted");
    assert.deepEqual(readFileSync(stage2Artifact), firstArtifact);
    assert.deepEqual(readFileSync(provenance), firstProvenance);
  } finally {
