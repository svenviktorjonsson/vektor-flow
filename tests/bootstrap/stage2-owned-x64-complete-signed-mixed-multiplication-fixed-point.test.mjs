...
test("Stage 2 multiplies a floating result by a negative integer through the complete x64 writer", {
  ...
  const work = mkdtempSync(join(rootWork, "i222-"));
  try {
    ...
    const divisionPath = join(work, "division.bin");
    const floatIntegerMultiplicationPath = join(work, "float-integer-multiplication.bin");
    const negatePath = join(work, "negate.bin");
    ...
    const floatIntegerMultiplication = Buffer.from([
      0x58, 0xf2, 0x48, 0x0f, 0x2a, 0xc8,
      0x58, 0x66, 0x48, 0x0f, 0x6e, 0xc0,
      0xf2, 0x0f, 0x59, 0xc1,
      0x66, 0x48, 0x0f, 0x7e, 0xc0, 0x50,
    ]);
    const negate = Buffer.from([0x58, 0x48, 0xf7, 0xd8, 0x50]);
    const floatingPrint = Buffer.from([
      0x58, 0x66, 0x48, 0x0f, 0x6e, 0xc0, 0xc3,
    ]);
    const generated = Buffer.concat([
      Buffer.from([0x6a, 0x08, 0x6a, 0x02]),
      division,
      Buffer.from([0x6a, 0x03]),
    ...
    writeFileSync(floatIntegerMultiplicationPath, floatIntegerMultiplication);
    writeFileSync(negatePath, negate);
    ...
    writeFileSync(inputSource, "value: 8\n:: (value / 2) * -----3\n", "utf8");
    ...
    assert.equal(oracle.stdout, `-12${newline}`);
    ...
    writeFileSync(stage2CompilerSource, [
      ...
      "float_integer_multiplication_path: io_stage.read_line()",
      "negate_path: io_stage.read_line()",
      ...
      "artifact: compiler_stage._compile_printed_dynamic_typed_multiplication_numeric_chain_x64(",
      "    io_stage.read_text(source_path),",
      "    io_stage.read_bytes(prefix_path),",
      "    io_stage.read_bytes(unused_path),",
      "    io_stage.read_bytes(unused_path),",
      "    io_stage.read_bytes(float_integer_multiplication_path),",
      "    io_stage.read_bytes(unused_path),",
      ...
      arenaReads.join(",\n"),
      "    ],",
      "    io_stage.read_bytes(negate_path)",
      ")",
    ...
    const compilerInput = [
      ...
      unusedPath,
      divisionPath,
      floatIntegerMultiplicationPath,
      negatePath,
      floatingPrintPath,
      byteArenaRoot,
    ];
    ...
    assert.deepEqual(
      ...
      generated,
      "complete-writer signed mixed-multiplication bytes differ from the selected tape",
    );
  } finally {
