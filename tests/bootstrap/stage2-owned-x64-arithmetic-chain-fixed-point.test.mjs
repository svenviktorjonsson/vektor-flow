...
test("Stage 2 traverses a compositional arithmetic tape at fixed point", {
  ...
  const work = mkdtempSync(join(rootWork, "i190-stage2-x64-chain-"));
  try {
    ...
    const inputSource = join(work, "valid-arithmetic-chain.vkf");
    ...
    assert.equal(oracle.stdout, `3050${newline}`);

    const stage2CompilerSource = join(work, "stage2-chain-compiler.vkf");
    const stage2Compiler = join(work, `stage2-chain-compiler${suffix}`);
    ...
    const stage2Program = join(work, `stage2-chain${suffix}`);
    const stage3Compiler = join(work, `stage3-chain-compiler${suffix}`);
    ...
    const stage3Program = join(work, `stage3-chain${suffix}`);
    const stage4Compiler = join(work, `stage4-chain-compiler${suffix}`);
    ...
    assert.deepEqual(
      readFileSync(stage2Program).subarray(markerOffset, markerOffset + 42),
      Buffer.from([
      ...
      "Stage 2 did not select the complete arithmetic instruction tape",
    );
  } finally {
