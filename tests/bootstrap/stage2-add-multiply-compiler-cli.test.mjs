...
const component = "machine_ir.closed_add_multiply.typed_module_pipeline";
...
test("Stage 2 compiler CLI preserves multiplication precedence", {
  ...
  const work = mkdtempSync(join(rootWork, "i159-stage2-add-multiply-"));
  try {
    ...
    const inputSource = join(work, "add-multiply-input.vkf");
    const oracleArtifact = join(work, `add-multiply-oracle${suffix}`);
    ...
    assert.equal(oracleRun.stdout, `33${newline}`);

    const stage2Artifact = join(work, `stage2-add-multiply${suffix}`);
    const observation = join(work, "stage2-add-multiply-observation.txt");
    const provenance = join(work, "stage2-add-multiply-provenance.json");
    const cliSource = join(work, "stage2-add-multiply-compiler.vkf");
    const cliArtifact = join(work, `stage2-add-multiply-compiler${suffix}`);
    ...
    writeFileSync(cliSource, [
      ...
      "module: compiler_stage.compile_tagged_printed_add_multiply(source)",
    ...
    assert.notEqual(rejected.status, 0, "unknown mixed binding was accepted");
    assert.deepEqual(readFileSync(stage2Artifact), firstArtifact);
    assert.deepEqual(readFileSync(provenance), firstProvenance);
  } finally {
