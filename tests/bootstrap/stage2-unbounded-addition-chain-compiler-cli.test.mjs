...
const component = "machine_ir.closed_dependency_chain.typed_module_pipeline";
...
test("Stage 2 compiler CLI parses an unbounded addition chain", {
  ...
  const work = mkdtempSync(join(rootWork, "i170-stage2-unbounded-addition-"));
  try {
    ...
    const inputSource = join(work, "unbounded-addition-input.vkf");
    const oracleArtifact = join(work, `unbounded-addition-oracle${suffix}`);
    writeFileSync(
      inputSource,
      "value: 31\n:: value + 1 + 2 + 3 + 4 + 5 + 6\n",
      "utf8",
    ...
    assert.equal(oracleRun.stdout, `52${newline}`);

    const stage2Artifact = join(work, `stage2-unbounded-addition${suffix}`);
    const observation = join(work, "stage2-unbounded-addition-observation.txt");
    const provenance = join(work, "stage2-unbounded-addition-provenance.json");
    const cliSource = join(work, "stage2-unbounded-addition-compiler.vkf");
    const cliArtifact = join(work, `stage2-unbounded-addition-compiler${suffix}`);
    writeFileSync(cliSource, [
      ...
      "statement: compiler_stage.compile_tagged_printed_dynamic_addition(source)",
    ...
    assert.notEqual(rejected.status, 0, "unknown unbounded-addition binding was accepted");
    assert.deepEqual(readFileSync(stage2Artifact), firstArtifact);
    assert.deepEqual(readFileSync(provenance), firstProvenance);
  } finally {
