...
test("Stage 2 compiler CLI preserves true-division precedence", {
  ...
  const work = mkdtempSync(join(rootWork, "i174-stage2-unbounded-divide-"));
  try {
    ...
    const inputSource = join(work, "unbounded-divide-input.vkf");
    const oracleArtifact = join(work, `unbounded-divide-oracle${suffix}`);
    writeFileSync(
      inputSource,
      "value: 100\n:: value - 25 / 4 * 2 + 9 / 4\n",
      "utf8",
    ...
    assert.equal(oracleRun.stdout, `89.75${newline}`);

    const stage2Artifact = join(work, `stage2-unbounded-divide${suffix}`);
    const observation = join(work, "stage2-unbounded-divide-observation.txt");
    const provenance = join(work, "stage2-unbounded-divide-provenance.json");
    const cliSource = join(work, "stage2-unbounded-divide-compiler.vkf");
    const cliArtifact = join(work, `stage2-unbounded-divide-compiler${suffix}`);
    ...
    assert.notEqual(rejected.status, 0, "unknown true-division binding was accepted");
    assert.deepEqual(readFileSync(stage2Artifact), firstArtifact);
    assert.deepEqual(readFileSync(provenance), firstProvenance);
  } finally {
