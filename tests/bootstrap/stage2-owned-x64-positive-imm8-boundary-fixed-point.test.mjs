...
test("Stage 2 encodes the positive imm8 boundary at fixed point", {
  ...
  const work = mkdtempSync(join(rootWork, "i194-stage2-x64-imm8-"));
  try {
    ...
    const generatedCodeBytes = 4 + addition.length + print.length;
    ...
    const inputSource = join(work, "valid-imm8-boundary.vkf");
    const oracleArtifact = join(work, `oracle${suffix}`);
    writeFileSync(inputSource, "value: 127\n:: value + 1\n", "utf8");
    ...
    assert.equal(oracle.stdout, `128${newline}`);

    const stage2CompilerSource = join(work, "stage2-imm8-compiler.vkf");
    const stage2Compiler = join(work, `stage2-imm8-compiler${suffix}`);
    writeFileSync(stage2CompilerSource, [
      ...
      "artifact: compiler_stage._compile_printed_dynamic_arithmetic_chain_x64(",
    ...
    const stage2Program = join(work, `stage2-imm8${suffix}`);
    const stage3Compiler = join(work, `stage3-imm8-compiler${suffix}`);
    ...
    const stage3Program = join(work, `stage3-imm8${suffix}`);
    const stage4Compiler = join(work, `stage4-imm8-compiler${suffix}`);
    ...
    assert.deepEqual(
      ...
      "Stage 2 did not emit the positive imm8 boundary exactly",
    );
  } finally {
