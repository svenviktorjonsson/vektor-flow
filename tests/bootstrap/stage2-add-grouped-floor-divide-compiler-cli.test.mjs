...
test("Stage 2 compiler CLI preserves parenthesized floor-division precedence", {
  ...
  const work = mkdtempSync(join(rootWork, "i166-stage2-grouped-floor-divide-"));
  try {
    ...
    const inputSource = join(work, "grouped-floor-divide-input.vkf");
    const oracleArtifact = join(work, `grouped-floor-divide-oracle${suffix}`);
    ...
    const stage2Artifact = join(work, `stage2-grouped-floor-divide${suffix}`);
    const observation = join(work, "stage2-grouped-floor-divide-observation.txt");
    const provenance = join(work, "stage2-grouped-floor-divide-provenance.json");
    const cliSource = join(work, "stage2-grouped-floor-divide-compiler.vkf");
    const cliArtifact = join(work, `stage2-grouped-floor-divide-compiler${suffix}`);
    ...
    writeFileSync(cliSource, [
      ...
      "module: compiler_stage.compile_tagged_printed_add_grouped_floor_divide(source)",
    ...
    assert.notEqual(rejected.status, 0, "unknown grouped floor-division binding was accepted");
    assert.deepEqual(readFileSync(stage2Artifact), firstArtifact);
    assert.deepEqual(readFileSync(provenance), firstProvenance);
  } finally {
