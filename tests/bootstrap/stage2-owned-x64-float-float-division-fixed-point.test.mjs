...
test("Stage 2 divides one grouped floating result by another", {
  ...
  const work = mkdtempSync(join(rootWork, "i213-"));
  try {
    ...
    const floatingDivisionPath = join(work, "float-divide.bin");
    ...
    writeFileSync(stage2CompilerSource, [
      ...
      "artifact: compiler_stage._compile_printed_dynamic_grouped_floating_division_numeric_chain_x64(",
    ...
    assert.deepEqual(
      ...
      generated,
      "float-divided-by-float bytes differ from the selected grouped tape",
    );
  } finally {
