...
test("Stage 2 emits a positive imm64 through the complete integer writer", {
  ...
  const work = mkdtempSync(join(rootWork, "i199-"));
  try {
    ...
    const generated = Buffer.concat([
      Buffer.from([0x48, 0xb8, 0x00, 0x00, 0x00, 0x80, 0x00, 0x00, 0x00, 0x00, 0x50]),
    ...
    writeFileSync(inputSource, "value: 2147483648\n:: value + 1\n", "utf8");
    ...
    assert.equal(oracle.stdout, `2147483649${newline}`);
    ...
    const compilerInput = [inputSource, prefixPath, additionPath, unusedPath, printTailPath, byteArenaRoot];
    ...
    assert.deepEqual(
      ...
      generated,
      "positive imm64 bytes differ from Stage 0 semantics",
    );
  } finally {
