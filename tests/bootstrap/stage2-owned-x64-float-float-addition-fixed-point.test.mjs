...
test("Stage 2 adds two division results in its typed numeric stack", {
  ...
  const work = mkdtempSync(join(rootWork, "i203-"));
  try {
    ...
    const floatingAdditionPath = join(work, "floating-add.bin");
    ...
    const floatingAddition = Buffer.from([
    ...
    const generated = Buffer.concat([
      Buffer.from([0x6a, 0x5a, 0x6a, 0x28]),
      division,
      Buffer.from([0x6a, 0x03, 0x6a, 0x02]),
      division,
      floatingAddition,
      floatingPrint,
    ]);
    ...
    writeFileSync(floatingAdditionPath, floatingAddition);
    ...
    assert.equal(oracle.stdout, `3.75${newline}`);
    ...
    writeFileSync(stage2CompilerSource, [
      ...
      "floating_addition_path: io_stage.read_line()",
      ...
      "    io_stage.read_bytes(unused_path),",
      "    io_stage.read_bytes(unused_path),",
      "    io_stage.read_bytes(floating_addition_path),",
      "    io_stage.read_bytes(unused_path),",
      "    io_stage.read_bytes(unused_path),",
    ...
    assert.deepEqual(
      ...
      generated,
      "float-plus-float bytes differ from the selected typed numeric tape",
    );
  } finally {
