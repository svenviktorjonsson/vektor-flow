...
test("Stage 2 compiler CLI preserves multiplication precedence in a dynamic chain", {
  ...
  const work = mkdtempSync(join(rootWork, "i172-stage2-unbounded-multiply-"));
  try {
    ...
    const inputSource = join(work, "unbounded-multiply-input.vkf");
    const oracleArtifact = join(work, `unbounded-multiply-oracle${suffix}`);
    writeFileSync(
      inputSource,
    ...
    assert.equal(oracleRun.stdout, `125${newline}`);

    const stage2Artifact = join(work, `stage2-unbounded-multiply${suffix}`);
    const observation = join(work, "stage2-unbounded-multiply-observation.txt");
    const provenance = join(work, "stage2-unbounded-multiply-provenance.json");
    const cliSource = join(work, "stage2-unbounded-multiply-compiler.vkf");
    const cliArtifact = join(work, `stage2-unbounded-multiply-compiler${suffix}`);
    writeFileSync(cliSource, [
      ...
      "statement: compiler_stage.compile_tagged_printed_dynamic_mixed_arithmetic(source)",
    ...
    writeFileSync(
      inputSource,
      "value: 100\n:: other + 24 // 3 * 2 + 7 * 5 // 4 + 1\n",
      "utf8",
    );
    const rejected = runCli();
    assert.notEqual(rejected.status, 0, "unknown multiply-precedence binding was accepted");
    assert.deepEqual(readFileSync(stage2Artifact), firstArtifact);
    assert.deepEqual(readFileSync(provenance), firstProvenance);
  } finally {
