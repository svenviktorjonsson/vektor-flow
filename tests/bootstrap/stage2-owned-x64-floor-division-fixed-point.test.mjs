...
test("Stage 2 emits runtime x64 floor division at bounded fixed point", {
  ...
  const work = mkdtempSync(join(rootWork, "i186-stage2-x64-floor-division-"));
  try {
    ...
    const tailPath = join(work, "floor-division-tail.bin");
    ...
    const inputSource = join(work, "valid-floor-division.vkf");
    ...
    const stage2CompilerSource = join(work, "stage2-floor-division-compiler.vkf");
    const stage2Compiler = join(work, `stage2-floor-division-compiler${suffix}`);
    writeFileSync(stage2CompilerSource, [
      ...
      "artifact: compiler_stage._compile_printed_dynamic_floor_division_expression_x64(",
    ...
    const stage2Program = join(work, `stage2-floor-division${suffix}`);
    const stage3Compiler = join(work, `stage3-floor-division-compiler${suffix}`);
    ...
    const stage3Program = join(work, `stage3-floor-division${suffix}`);
    const stage4Compiler = join(work, `stage4-floor-division-compiler${suffix}`);
    ...
    assert.deepEqual(
      readFileSync(stage2Program).subarray(markerOffset, markerOffset + 17),
      Buffer.from([
        0x6a, 0x5a, 0x6a, 0x28,
        0x59, 0x58, 0x48, 0x99, 0x48, 0xf7, 0xf9,
        0xf2, 0x48, 0x0f, 0x2a, 0xc0, 0xc3,
      ]),
      "Stage 2 did not emit the runtime floor-division instruction sequence",
    );
  } finally {
