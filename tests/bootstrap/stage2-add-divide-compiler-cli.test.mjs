...
const component = "machine_ir.closed_add_divide.typed_module_pipeline";
...
test("Stage 2 compiler CLI preserves division precedence", {
  ...
  const work = mkdtempSync(join(rootWork, "i161-stage2-add-divide-"));
  try {
    ...
    const inputSource = join(work, "add-divide-input.vkf");
    const oracleArtifact = join(work, `add-divide-oracle${suffix}`);
    ...
    const stage2Artifact = join(work, `stage2-add-divide${suffix}`);
    const observation = join(work, "stage2-add-divide-observation.txt");
    const provenance = join(work, "stage2-add-divide-provenance.json");
    const cliSource = join(work, "stage2-add-divide-compiler.vkf");
    const cliArtifact = join(work, `stage2-add-divide-compiler${suffix}`);
    ...
    writeFileSync(cliSource, [
      ...
      "module: compiler_stage.compile_tagged_printed_add_divide(source)",
    ...
    assert.notEqual(rejected.status, 0, "unknown division binding was accepted");
    assert.deepEqual(readFileSync(stage2Artifact), firstArtifact);
    assert.deepEqual(readFileSync(provenance), firstProvenance);
  } finally {
