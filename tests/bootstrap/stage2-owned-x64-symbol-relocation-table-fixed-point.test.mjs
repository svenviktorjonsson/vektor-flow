...
test("Stage 2 discovers function symbol positions before applying x64 relocations", {
  ...
  const work = mkdtempSync(join(rootWork, "i227-"));
  try {
    ...
    const helper = Buffer.concat([
      functionPrologue,
      Buffer.from([0x6a, 0x02]),
      multiplication,
      functionEpilogue,
    ]);
    assert.equal(helper.length, 20, "resolved entry displacement changed");
    const entryOffset = 2 + helper.length;
    const callReturnOffset = entryOffset + 2 + 5;
    assert.equal(2 - callReturnOffset, -27, "resolved backward displacement changed");
    const generated = Buffer.concat([
      Buffer.from([0xeb, 0x14]),
      helper,
      Buffer.from([0x6a, 0x15]),
      Buffer.from([0xe8, 0xe5, 0xff, 0xff, 0xff]),
      Buffer.from([0xc3]),
    ]);
    ...
    copyFileSync(join(root, "examples", "native_core", "hello_native.vkf"), inputSource);
    ...
    writeFileSync(stage2CompilerSource, [
      ...
      "artifact: compiler_stage._compile_tagged_numeric_literal_function_symbols_x64(",
    ...
    assert.doesNotMatch(
      readFileSync(stage2CompilerSource, "utf8"),
      /\n\s+(?:20|-27),/,
      "caller must not supply resolved relocation positions",
    );
    ...
    assert.deepEqual(
      ...
      generated,
      "symbol-resolved relocation bytes differ from the selected artifact",
    );
  } finally {
