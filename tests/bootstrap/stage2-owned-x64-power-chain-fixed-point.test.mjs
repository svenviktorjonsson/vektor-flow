...
test("Stage 2 composes integer power within a tape at fixed point", {
  ...
  const work = mkdtempSync(join(rootWork, "i193-stage2-x64-power-chain-"));
  try {
    ...
    const inputSource = join(work, "valid-power-chain.vkf");
    ...
    assert.equal(oracle.stdout, `1665${newline}`);

    const stage2CompilerSource = join(work, "stage2-power-chain-compiler.vkf");
    const stage2Compiler = join(work, `stage2-power-chain-compiler${suffix}`);
    writeFileSync(stage2CompilerSource, [
      ...
      "artifact: compiler_stage._compile_printed_dynamic_power_chain_x64(",
    ...
    const stage2Program = join(work, `stage2-power-chain${suffix}`);
    const stage3Compiler = join(work, `stage3-power-chain-compiler${suffix}`);
    ...
    const stage3Program = join(work, `stage3-power-chain${suffix}`);
    const stage4Compiler = join(work, `stage4-power-chain-compiler${suffix}`);
    ...
    assert.deepEqual(
      readFileSync(stage2Program).subarray(markerOffset, markerOffset + 41),
      Buffer.from([
        0x6a, 0x28, 0x6a, 0x02,
        0x59, 0x58,
        0xba, 0x01, 0x00, 0x00, 0x00,
        ...
        0x48, 0x0f, 0xaf, 0xd0,
        0x48, 0xff, 0xc9, 0x75, 0xf7, 0x52,
        0x6a, 0x41,
        0x58, 0x59, 0x48, 0x01, 0xc8, 0x50,
        0x58, 0xf2, 0x48, 0x0f, 0x2a, 0xc0, 0xc3,
      ]),
      "Stage 2 did not compose integer power within the instruction tape",
    );
  } finally {
