...
test("Stage 2 compiler CLI preserves remainder precedence and associativity", {
  ...
  const work = mkdtempSync(join(rootWork, "i175-stage2-unbounded-remainder-"));
  try {
    ...
    const inputSource = join(work, "unbounded-remainder-input.vkf");
    const oracleArtifact = join(work, `unbounded-remainder-oracle${suffix}`);
    writeFileSync(
      inputSource,
      "value: 100\n:: value - 29 % 6 * 3 + 11 % 4\n",
      "utf8",
    ...
    assert.equal(oracleRun.stdout, `88${newline}`);

    const stage2Artifact = join(work, `stage2-unbounded-remainder${suffix}`);
    const observation = join(work, "stage2-unbounded-remainder-observation.txt");
    const provenance = join(work, "stage2-unbounded-remainder-provenance.json");
    const cliSource = join(work, "stage2-unbounded-remainder-compiler.vkf");
    const cliArtifact = join(work, `stage2-unbounded-remainder-compiler${suffix}`);
