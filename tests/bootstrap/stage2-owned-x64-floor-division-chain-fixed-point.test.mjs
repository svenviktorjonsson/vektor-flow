...
test("Stage 2 composes floor division within an integer tape at fixed point", {
  ...
  const work = mkdtempSync(join(rootWork, "i192-stage2-x64-floor-chain-"));
  try {
    ...
    const floorDivisionPath = join(work, "floor-division.bin");
    ...
    const floorDivision = Buffer.from([
      0x59,
      0x58,
      0x48, 0x99,
      0x48, 0xf7, 0xf9,
    ...
    const inputSource = join(work, "valid-floor-division-chain.vkf");
    ...
    assert.equal(oracle.stdout, `67${newline}`);

    const stage2CompilerSource = join(work, "stage2-floor-chain-compiler.vkf");
    const stage2Compiler = join(work, `stage2-floor-chain-compiler${suffix}`);
    writeFileSync(stage2CompilerSource, [
      ...
      "artifact: compiler_stage._compile_printed_dynamic_floor_division_chain_x64(",
    ...
    const stage2Program = join(work, `stage2-floor-chain${suffix}`);
    const stage3Compiler = join(work, `stage3-floor-chain-compiler${suffix}`);
    ...
    const stage3Program = join(work, `stage3-floor-chain${suffix}`);
    const stage4Compiler = join(work, `stage4-floor-chain-compiler${suffix}`);
    ...
    assert.deepEqual(
      ...
      Buffer.from([
        0x6a, 0x5a, 0x6a, 0x28,
        0x59, 0x58, 0x48, 0x99, 0x48, 0xf7, 0xf9, 0x50,
        0x6a, 0x41,
        0x58, 0x59, 0x48, 0x01, 0xc8, 0x50,
        0x58, 0xf2, 0x48, 0x0f, 0x2a, 0xc0, 0xc3,
      ]),
      "Stage 2 did not compose floor division within the instruction tape",
    );
  } finally {
