...
constexpr const char* vkf_release_version = "0.4.0";
...
using TaggedTest = vkf::testing::TaggedTest;
...
std::vector<std::filesystem::path> test_source_files(const std::filesystem::path& target) {
    ...
    if (std::filesystem::is_regular_file(target)) {
        if (!vkf::testing::is_test_source_file(target.generic_string(), false)) {
        ...
        for (const auto& entry : std::filesystem::recursive_directory_iterator(target)) {
            if (entry.is_regular_file()) regular_files.push_back(entry.path().generic_string());
        }
        for (const auto& file : vkf::testing::select_test_source_files(regular_files)) files.emplace_back(file);
    } else {
...
using vkf::module_linker::dot_module_name;
...
vf::JsonValue link_spilled_file_modules(
    ...
    provider.resolve = [](const std::string& importing_source, const std::string& module)
        -> std::optional<std::string> {
        const auto resolved = resolve_dot_module(importing_source, module);
        return resolved ? std::optional<std::string>(resolved->string()) : std::nullopt;
    };
    provider.parse = [&](const std::string& source) {
        return parse_linked_module(source, cache_stats);
    };
    provider.canonical = [](const std::string& source) {
        return std::filesystem::absolute(source).lexically_normal().string();
    };
    try {
        return vkf::module_linker::Linker(provider).link(root_module, root_source.string());
    } catch (const vkf::module_linker::ModuleLinkError& error) {
        throw DriverFailure(error.what());
    }
}
...
std::vector<TaggedTest> discover_tests(
    ...
    try {
        return vkf::testing::discover_tests(source, file.string());
    } catch (const vkf::testing::TestSuiteFailure& error) {
        throw DriverFailure(error.what());
    }
}
#endif
    ...
    try {
        ...
        if (argc >= 2 && std::string(argv[1]) == "-t") {
            ...
            for (const auto& file : test_source_files(argv[2])) {
                ...
                const auto compile_error = vkf::testing::expected_compile_error(source);
                if (compile_error) {
                    ++discovered;
                    const std::string& expected = *compile_error;
                    ...
                    try {
                        ...
                        if (vkf::testing::matches_compile_error(expected, actual)) {
                            ++passed;
                            std::cout << "PASS " << label << '\n';
                        } else {
                            ++failed;
                ...
                for (const auto& test : discover_tests(source, file)) {
                    ...
                    const std::string generated = vkf::testing::test_entry_source(source, test.name);
