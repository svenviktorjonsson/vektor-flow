...
using output_effects::collect_error_effects;
...
inline ValueLayout projected_layout(
    ...
    result.dynamic_element = selected.dynamic_element;
...
inline bool expression_produces_owned_f64_list(
    ...
    if (kind == "list") {
        ...
        return type != expression.end() && type->second.is_string() &&
            layout_from_type(type->second.as_string(), &signatures).kind ==
                ValueKind::DynamicF64List;
    }
    if (kind == "axis_align") {
...
[[gnu::noinline]] inline ValueLayout emit_mixed_f64_list_concat(
    ...
    const ValueLayout& dynamic_layout,
...
inline ValueLayout lower_expression(
    ...
    if (kind == "list" || kind == "tuple") {
        ...
        const ValueLayout declared = kind == "list" && type != expression.end() &&
                type->second.is_string()
            ? layout_from_type(type->second.as_string(), &signatures)
            : ValueLayout{};
        const bool dynamic = declared.kind == ValueKind::DynamicF64List;
        ...
        for (const auto& value : items) {
            ...
            if (dynamic) {
                if (spread) {
                    if (element.kind != ValueKind::Aggregate || is_record_layout(element)) {
                        throw LoweringFailure(
                            "dynamic list literal can only spread a fixed collection");
                    }
                    ...
                    for (const auto& spread_element : spread_elements) {
                        if (declared.dynamic_element &&
                            !same_layout(spread_element, *declared.dynamic_element)) {
                            throw LoweringFailure("dynamic list spread element layout mismatch");
                        }
                    }
                    elements.insert(
                ...
                if (declared.dynamic_element &&
                    !same_layout(element, *declared.dynamic_element)) {
                    throw LoweringFailure("dynamic list element layout mismatch");
                }
        ...
        if (dynamic) {
            Instruction make;
            make.opcode = Opcode::MakeOwnedF64List;
            make.argument_count = std::accumulate(
                elements.begin(), elements.end(), std::uint32_t{0},
                [](std::uint32_t width, const ValueLayout& element) {
    ...
    if (kind == "block_expr") {
        ...
        for (std::size_t index = 0; index < body.size(); ++index) {
            ...
            if (statement_kind == "store_binding") {
                ...
                const auto value_layout = lower_expression(value, builder, signatures, strings);
                ensure_independent_value(value, value_layout, builder, signatures);
                // The frontend resolves this initializer before introducing
                // the declaration. Preserve that lexical environment until
                // its value (including borrowed resources) has been captured.
    ...
    if (kind == "dotted_index" || kind == "field_access") {
        ...
        if (kind == "dotted_index") {
            ...
            if (base_layout.kind == ValueKind::DynamicF64List) {
                ...
                lowered_index = emit_require_real_complex(
                    builder, strings, lowered_index, "index must be int or str");
                require_scalar(lowered_index, "dynamic list index");
                if (base_layout.dynamic_element) {
                    const auto index_local = builder.add_borrowed_temporary({});
                    const auto base_local = builder.add_borrowed_temporary(base_layout);
                    builder.emit({Opcode::StoreLocal, 0.0, index_local});
                    builder.emit({Opcode::StoreLocal, 0.0, base_local});
                    const auto& element = *base_layout.dynamic_element;
                    for (std::uint32_t component = 0; component < element.width; ++component) {
                        builder.emit({Opcode::LoadLocal, 0.0, base_local});
                        builder.emit({Opcode::LoadLocal, 0.0, index_local});
                        Instruction stride;
                        stride.opcode = Opcode::PushF64;
                        stride.f64 = static_cast<double>(element.width);
                        builder.emit(std::move(stride));
                        builder.emit({Opcode::MultiplyF64});
                        if (component != 0u) {
                            Instruction offset;
                            offset.opcode = Opcode::PushF64;
                            offset.f64 = static_cast<double>(component);
                            builder.emit(std::move(offset));
                            builder.emit({Opcode::AddF64});
                        }
                        Instruction index;
                        index.opcode = Opcode::LoadF64ListIndex;
                    ...
                    if (owns_input) {
                        const auto result_local = builder.add_borrowed_temporary(element);
                        for (std::uint32_t component = element.width; component > 0; --component) {
                            builder.emit({Opcode::StoreLocal, 0.0, result_local + component - 1u});
                        }
                        builder.emit({Opcode::ReleaseF64ListLocal, 0.0, base_local});
                        for (std::uint32_t component = 0; component < element.width; ++component) {
                            builder.emit({Opcode::LoadLocal, 0.0, result_local + component});
                        }
                    }
                    return element;
                }
        ...
        if (kind == "field_access") {
            const auto& object = object_of(
                field(expression, "object", "field access"), "field access object");
            if (string_field(object, "kind", "field access object") == "dotted_index") {
                const auto object_layout = layout_from_builder_expression_shape(
                    object, builder, signatures);
                const std::string field_name = string_field(
                    expression, "field", "field access");
                const auto selected = object_layout.selectors.find(field_name);
                if (selected != object_layout.selectors.end()) {
                    const auto lowered = lower_expression(
                        object, builder, signatures, strings);
                    if (!same_layout(lowered, object_layout)) {
                        throw LoweringFailure("computed field access layout mismatch");
                    }
                    const auto temporary = builder.add_borrowed_temporary(object_layout);
                    for (std::uint32_t component = object_layout.width;
                         component > 0; --component) {
                        builder.emit({
                            Opcode::StoreLocal, 0.0,
                            temporary + component - 1u});
                    }
                    for (std::uint32_t component = 0;
                         component < selected->second.width; ++component) {
                        builder.emit({
                            Opcode::LoadLocal, 0.0,
                            temporary + selected->second.offset + component});
                    }
                    return record_field_layout(
                        object_layout, field_name, selected->second);
                }
            }
        }
        const auto projection = projection_of(expression);
    ...
    if (kind == "binary_op") {
        ...
        if (op == "AMPERSAND" && left.kind == ValueKind::DynamicF64List &&
            ...
            return emit_mixed_f64_list_concat(
                true, right, left, left_expression, builder, signatures);
        }
        if (op == "AMPERSAND" && right.kind == ValueKind::DynamicF64List &&
            left.kind == ValueKind::Aggregate && !is_record_layout(left)) {
            return emit_mixed_f64_list_concat(
                false, left, right, right_expression, builder, signatures);
        }
        if (op == "AMPERSAND" && left.kind == ValueKind::DynamicF64List &&
    ...
    if (kind == "call") {
        ...
        if (callee_kind == "field_access" &&
            ...
            if (source_layout.kind == ValueKind::DynamicF64List) {
                ...
                builder.emit(std::move(count));
                if (source_layout.dynamic_element &&
                    source_layout.dynamic_element->width > 1u) {
                    Instruction stride;
                    stride.opcode = Opcode::PushF64;
                    stride.f64 = static_cast<double>(source_layout.dynamic_element->width);
                    builder.emit(std::move(stride));
                    builder.emit({Opcode::DivideF64});
                }
                return {};
        ...
        if (callee_kind == "stdlib_function") {
            ...
            const std::uint32_t degrees_of_freedom = module == "stat"
                ? StatValidation::degrees_of_freedom(name, named_args) : 0;
            if (module != "stat" && !named_args.empty()) {
                throw LoweringFailure("direct machine IR stdlib call does not accept named arguments " + module + "." + name);
            }
            ...
            if (module == "stat" &&
                ...
                if (argument_kind == "load" || argument_kind == "field_access" ||
                    ...
                    const auto& source_layout = builder.layout(projection.binding);
                    ValueSlice slice{
                        0, source_layout.width, source_layout.kind,
                        source_layout.dynamic_element};
        ...
        if (signature == signatures.end() &&
            vkf::math_primitives::classify(symbol) != vkf::math_primitives::Kind::None) {
            vkf::math_primitives::validate_builtin<LoweringFailure>(symbol, args.size(),
                array_of(field(expression, "named_args", "call"), "named call args").size(),
                array_of(field(expression, "spread_args", "call"), "spread call args").size());
            const auto argument = lower_expression(
        ...
        try {
            positional_placement = call_binding::plan_positional_call(
                parameter_values.size(), args.size(), variadic_index, symbol);
        } catch (const call_binding::Error& error) {
            throw LoweringFailure(error.what());
        }
        const std::size_t fixed_positional_count = positional_placement.fixed_count;
        ...
        if (variadic_index) {
            for (std::size_t index = positional_placement.rest_begin; index < args.size(); ++index) {
        ...
        // Share fixed-call placement only. Operand evaluation, missing-default
        // diagnostics, variadic packing, and spread lowering stay below in
        // their existing native order. Do not eagerly validate later operands.
        std::vector<call_binding::Parameter> fixed_parameters;
        std::optional<call_binding::FixedCallPlan> fixed_call_plan;
        const auto fixed_spread = expression.find("spread_args");
        if (!variadic_index && !variadic_named_index &&
            fixed_spread != expression.end() && fixed_spread->second.is_array() &&
            fixed_spread->second.as_array().empty()) {
            for (std::size_t index = 0; index < parameter_values.size(); ++index) {
                fixed_parameters.push_back({signature->second.parameter_names[index],
                    index < signature->second.parameter_defaults.size() &&
                    signature->second.parameter_defaults[index] != nullptr});
            }
            try {
                fixed_call_plan = call_binding::plan_fixed_call(
                    fixed_parameters, args.size(), {}, symbol);
            } catch (const call_binding::Error& error) {
                throw LoweringFailure(error.what());
            }
        }
        for (std::size_t named_index = 0; named_index < named_args.size(); ++named_index) {
            const auto& named_value = named_args[named_index];
            ...
            if (fixed_call_plan) {
                std::size_t index = 0;
                try {
                    index = call_binding::bind_named_argument(
                        *fixed_call_plan, fixed_parameters, name, named_index, symbol);
                } catch (const call_binding::Error& error) {
                    throw LoweringFailure(error.what());
                }
        ...
        if (!spread_args.empty() && !variadic_index) {
            ...
            std::vector<call_binding::FixedSpreadParameter> spread_parameters;
            for (std::size_t index = 0; index < parameter_values.size(); ++index) {
                spread_parameters.push_back({signature->second.parameter_names[index],
                    signature->second.parameters[index], parameter_values[index] != nullptr,
                    variadic_named_index && index == *variadic_named_index});
            }
            const auto spread_plan = call_binding::plan_fixed_spread(spread_layout, spread_parameters, symbol);
            if (spread_plan.dynamic_list) {
                Instruction load;
                load.opcode = Opcode::LoadLocal;
                load.index = temporary;
                builder.emit(std::move(load));
                ...
                Instruction expected_count;
                expected_count.opcode = Opcode::PushF64;
                expected_count.f64 = static_cast<double>(spread_plan.required_count);
                builder.emit(std::move(expected_count));
                builder.emit({Opcode::OrderedEqualF64});
                Instruction arity;
                arity.opcode = Opcode::AssertTruthy;
            ...
            for (const auto& [index, selection] : spread_plan.parameters) {
                fixed_spread_components[index] = {temporary, selection.slice, selection.layout,
                    spread_plan.dynamic_list, selection.index};
            }
        }
        for (std::size_t index = 0; index < parameter_values.size(); ++index) {
...
using module_snapshots::substitute_closure_loads;
using module_snapshots::collect_function_local_names;

inline std::optional<vf::JsonValue> capture_module_literal_snapshots(const vf::JsonValue& typed_ir) {
    try {
        return module_snapshots::capture_module_literal_snapshots(typed_ir);
...
inline Module lower_monomorphic(const vf::JsonValue& typed_ir) {
    ...
    auto inferred_layouts = infer_module_layouts(typed_ir);
    auto& functions = inferred_layouts.functions;
    auto& signatures = inferred_layouts.signatures;
    auto& entry_statements = inferred_layouts.entry_statements;
    ...
    const auto reachable_functions = output_effects::reachable_functions_for(
        functions, function_callees, entry_statements);
    ...
    const auto is_print_statement = [](const vf::JsonValue::Object& statement) {
        return output_effects::is_print_statement<LoweringFailure>(statement);
    };
    ...
    // Entry return-value batching is valid only when no other output effects
    // can occur between those values. Nested prints must commit in source order.
    const bool has_nested_output_effect = output_effects::has_nested_output_effect<LoweringFailure>(
        functions, reachable_functions, entry_statements);
    if (has_custom_display_output || has_nested_output_effect) {
        for (const auto* statement : entry_statements) {
            ...
            const auto shape = display_shape_from_expression(
                printed, display_environment, &function_displays);
            if (custom_display_type.empty() || shape.kind != DisplayKind::Record ||
                shape.label != custom_display_type) {
                vf::JsonValue::Array single_statement;
                single_statement.emplace_back(*statement);
                lower_statements(
                    single_statement, entry, false, signatures, strings,
...
inline bool uses_fixed_local_storage(const Instruction& instruction, std::uint32_t local) {
    switch (instruction.opcode) {
        case Opcode::LoadF64LocalsIndex:
        case Opcode::StoreF64LocalsIndex:
    ...
    for (std::size_t pass = 0; pass < function.locals.size(); ++pass) {
        ...
        for (std::uint32_t destination = static_cast<std::uint32_t>(
            ...
            for (std::size_t position = 0; position < function.instructions.size(); ++position) {
                ...
                const bool fixed_storage_use = uses_fixed_local_storage(instruction, destination);
    ...
    for (;;) {
        ...
        for (std::uint32_t local = static_cast<std::uint32_t>(
            ...
            for (std::size_t position = 0;
                ...
                if (uses_fixed_local_storage(instruction, local)) {
                    ++writes;
                }
