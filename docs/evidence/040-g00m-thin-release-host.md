...
.\scripts\build-native-ui-package.ps1 `
  -BuildDirectory build/v -Target vf-overlay `
  -Configuration Release -Parallel 1
...
Both maps report `forbiddenSemanticObjects: []`. The standalone `vf-overlay`
release target also rebuilt successfully from the same thin source set. None of
the three PEs imports an MSVC redistributable; they import only `ADVAPI32.dll`,
`dbghelp.dll`, `GDI32.dll`, `KERNEL32.dll`, `ole32.dll`, `SHELL32.dll`, and
`USER32.dll`.
...
| Artifact | SHA-256 |
| --- | --- |
| `build/v/vf-overlay.exe` | `626BED653671F5064A4A8A382797B196E8EA2A092C8A5FCC5C0B48390F0AE387` |
