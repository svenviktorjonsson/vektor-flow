...
| `physics` | collision, contact, material, rigid-body mass, inertia, momentum, and deterministic example algorithms | typed numeric vector/matrix layout and dimension checking | none | 28 direct engine tests plus native/WASM same-source conformance; packaged as `physics` with no separate `rigid_body` module |
| `physics.units` | compact SI catalog declaration | typed unit synthesis, prefix expansion, and seven-exponent dimension arithmetic | none | 6 direct dimension/unit tests plus a compile-error mismatch test; default and explicit SI imports are packaged |
| `symbolic` | DAG algebra, polynomials, expansion, collection, simplification, substitution, calculus, evaluation, and series algorithms | canonical symbolic value layout and operators | real elementary math ABI only through `math` | 110 direct behavioral tests plus native/WASM same-source conformance; four published comparison kernels pass every `<2×` gate |
