...
function n_body(steps)
    ...
    b[1,4:6] .= -vec(sum(b[:,4:6] .* b[:,7], dims=1)) ./ solar
    for _ in 1:steps
        for i in 1:5, j in i+1:5
            d=b[i,1:3]-b[j,1:3]; squared=dot(d,d); magnitude=0.01/(squared*sqrt(squared))
            b[i,4:6] .-= d.*b[j,7].*magnitude; b[j,4:6] .+= d.*b[i,7].*magnitude
        end
        b[:,1:3] .+= 0.01 .* b[:,4:6]
    end
    energy=0.0
    for i in 1:5
        energy += 0.5*b[i,7]*dot(b[i,4:6],b[i,4:6])
        for j in i+1:5; d=b[i,1:3]-b[j,1:3]; energy -= b[i,7]*b[j,7]/sqrt(dot(d,d)); end
    end
    energy
end

println(repr(n_body(50000)))
