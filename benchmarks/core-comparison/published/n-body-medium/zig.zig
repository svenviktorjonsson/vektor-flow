...
export fn vkf_benchmark() callconv(.c) f64 {
    ...
    for(0..10000)|_|{for(0..5)|i|{for(i+1..5)|j|{const dx=x[i]-x[j];const dy=y[i]-y[j];const dz=z[i]-z[j];const squared=dx*dx+dy*dy+dz*dz;const magnitude=0.01/(squared*@sqrt(squared));vx[i]-=dx*mass[j]*magnitude;vy[i]-=dy*mass[j]*magnitude;vz[i]-=dz*mass[j]*magnitude;vx[j]+=dx*mass[i]*magnitude;vy[j]+=dy*mass[i]*magnitude;vz[j]+=dz*mass[i]*magnitude;}}for(0..5)|i|{x[i]+=0.01*vx[i];y[i]+=0.01*vy[i];z[i]+=0.01*vz[i];}}
