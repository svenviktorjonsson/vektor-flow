# VKF 0.2.1 Optimizer Policy Landscape

![Sorted optimizer policy landscape](./windows-x64-v0.2.1-ci.svg)
...
| Search time | 98036.3 ms |
| Slowest policy mean | 12.192 ms |
| Fastest measured policy | `mask-4e` |
| Fastest measured mean | 2.291 ± 0.081 ms |
| Default `mask-ff` mean | 2.318 ± 0.098 ms |
| Fastest/slowest spread | 5.32× |
| Selected/default difference | 1.2% |

The complete machine-readable evidence, including all 256 policy records, hashes, output, compiler identity, and host conditions, is [windows-x64-v0.2.1-ci.json](./windows-x64-v0.2.1-ci.json).
...
The robust result is the large policy landscape: the best basin is about 5.32× faster than the slowest legal policy. The exact 1.2% lead of `mask-4e` over `mask-ff` is smaller than run-to-run variance and was not stable in a separate order-reversed check. It is a measured winner, not proof that it is universally faster. Release defaults therefore remain conservative, while profiles and future selectors can learn from the larger, repeatable policy effects.

## Reproduce

```powershell
node benchmarks/policy-landscape/run.mjs --compiler=build/native-compiler-clang/bin/vkf-strict.exe --runs=200 --output=benchmarks/policy-landscape/evidence/windows-x64-v0.2.1
```
