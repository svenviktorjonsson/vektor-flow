# VKF 0.1.6 Optimizer Policy Landscape

![Sorted optimizer policy landscape](./windows-x64-v0.1.6-ci.svg)
...
| Search time | 59986.5 ms |
| Slowest policy mean | 12.312 ms |
| Fastest measured policy | `mask-8e` |
| Fastest measured mean | 2.308 ± 0.082 ms |
| Default `mask-ff` mean | 2.308 ± 0.082 ms |
| Fastest/slowest spread | 5.34× |
| Selected/default difference | 0.0% |

The complete machine-readable evidence, including all 256 policy records, hashes, output, compiler identity, and host conditions, is [windows-x64-v0.1.6-ci.json](./windows-x64-v0.1.6-ci.json).
...
The robust result is the large policy landscape: the best basin is about 5.34× faster than the slowest legal policy. The exact 0.0% lead of `mask-8e` over `mask-ff` is smaller than run-to-run variance and was not stable in a separate order-reversed check. It is a measured winner, not proof that it is universally faster. Release defaults therefore remain conservative, while profiles and future selectors can learn from the larger, repeatable policy effects.

## Reproduce

```powershell
node benchmarks/policy-landscape/run.mjs --compiler=build/native-compiler-clang/bin/vkf-strict.exe --runs=200 --output=benchmarks/policy-landscape/evidence/windows-x64-v0.1.6
```
