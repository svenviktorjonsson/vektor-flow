  ...
  function createVkfSharedRectProgram() {
    ...
    return {
      ...
      update: function (input, api) {
        ...
        if (ui.keyboard.modifiers.ctrl) {
          switch (e.hover.kind) {
            case ui.HOVER_VERTEX:
              activeTarget.move_vertex({ vertex: e.hover.vertex_id, local_cursor: e.local_cursor, local_trans: e.local_trans });
              break;
            case ui.HOVER_EDGE:
              activeTarget.translate_edge({ edge: e.hover.edge_id, local_trans: e.local_trans });
              break;
          }
          return;
        }
