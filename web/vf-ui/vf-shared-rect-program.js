    ...
    return {
      ...
      init: function (api) {
        ...
        for (var i = 0; i < roots.length; i++) {
          var root = roots[i];
          var rootMesh = addQuad(panel, root, root, null, { vertex: 4, edge: 2 });
          var childSpec = quadInside(root, 1);
          var child = addQuad(rootMesh, root, childSpec, root.hidden, { vertex: 4, edge: 2 });
          var remaining = ["face", "edge", "vertex"].filter(function (kind) { return kind !== root.hidden; });
          addQuad(child, root, quadInside(root, 2, "left"), remaining[0], { vertex: 4, edge: 2 });
          addQuad(child, root, quadInside(root, 2, "right"), remaining[1], { vertex: 4, edge: 2 });
        }
      },
