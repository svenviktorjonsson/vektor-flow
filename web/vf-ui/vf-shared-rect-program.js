(function (global) {
  ...
  function quad(level, side) {
    if (level === 1) {
      return { x: [52, 168, 160, 60], y: [44, 54, 156, 164], origin: [110, 100, 0] };
    }
    if (side === "left") {
      return { x: [16, 58, 53, 21], y: [18, 22, 54, 59], origin: [37, 36, 0] };
    }
    return { x: [78, 122, 117, 83], y: [48, 53, 86, 90], origin: [100, 67, 0] };
  }

  function addQuad(parent, root, spec, hidden) {
    ...
    return {
      ...
      init: function (api) {
        ...
        for (var i = 0; i < roots.length; i++) {
          var root = roots[i];
          var rootMesh = addQuad(panel, root, root, null);
          var child = addQuad(rootMesh, root, quad(1), root.hidden);
          var remaining = ["face", "edge", "vertex"].filter(function (kind) { return kind !== root.hidden; });
          addQuad(child, root, quad(2, "left"), remaining[0]);
          addQuad(child, root, quad(2, "right"), remaining[1]);
        }
      },
